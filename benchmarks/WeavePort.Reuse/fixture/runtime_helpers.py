...
def wipe():
    # Only the disposable image's writable directories; callers enforce the container guard.
...
def invoke(request):
    ...
    os.umask(mask)
    before_signals = {
        s: signal.getsignal(s) for s in (signal.SIGUSR1, signal.SIGUSR2, signal.SIGALRM)
    }

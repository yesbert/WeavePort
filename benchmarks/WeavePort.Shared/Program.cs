...
var languages = new[]
{
    (Language: "python", Executable: python, Fixture: Path.Combine(root, "tests/WeavePort.Shared.Tests/fixtures/shared.py")),
    (Language: "typescript", Executable: node, Fixture: Path.Combine(root, "tests/WeavePort.Shared.Tests/fixtures/shared.mjs")),
    (Language: "csharp", Executable: FindExecutable("dotnet"), Fixture: csharp)
};
var scenarios = from language in languages
                from workload in new[] { "delay", "cpu" }
                from degree in new[] { 1, 16 }
                select (language, workload, degree);
foreach (var (language, workload, degree) in scenarios)
{
    measurements.Add(await MeasureAsync(language.Language, language.Executable, language.Fixture, workload, degree));
}
DateTimeOffset recordedUtc = TimeProvider.System.GetUtcNow();
string path = Path.Combine(root, "reports/benchmarks", $"shared-{recordedUtc:yyyyMMdd-HHmmss}.json");
...
static Dictionary<string, string> SourceHashes(string root)
{
    var areas = new[]
    {
        (Directory: "src/WeavePort.Hosting", Pattern: "*.cs"),
        (Directory: "sdks/python/weaveport_sdk", Pattern: "*.py"),
        (Directory: "sdks/typescript/src", Pattern: "*.ts")
    };
    return areas
        .SelectMany(area => Directory.EnumerateFiles(Path.Combine(root, area.Directory), area.Pattern, SearchOption.AllDirectories))
...
static async Task<object> MeasureAsync(string language, string executable, string fixture, string workload, int degree)
{
    ...
    int pid = (await probe.CallAsync("echo", Json(new
    {
    }))).GetProperty("pid").GetInt32();
    using var process = Process.GetProcessById(pid);
    JsonElement input = workload == "delay" ? Json(new
    {
        milliseconds = 150
    }) : Json(new
    {
    ...
    await using var sampler = new WorkerMemorySampler(process);
        ...
        await Task.WhenAll(clients.Select(async (client, i) =>
    {
        ...
        if (tenant != $"tenant-{i}")
        {
            throw new InvalidDataException("Tenant mismatch in benchmark.");
        }

        if (workload == "cpu" && result.GetProperty("digest").GetString() != expectedDigest)
        {
            throw new InvalidDataException("CPU result integrity failure.");
        }
    ...
    Console.WriteLine($"{language,-10} {workload,-5} degree={degree,2}: {throughput:F1} calls/s p95={latencies[(int)Math.Ceiling(count * .95) - 1]:F1}ms RSS={sampler.PeakBytes / 1048576d:F1}MiB");
    return new
    {
        language,
        workload,
        ...
        p50Ms = latencies[count / 2],
        p95Ms = latencies[(int)Math.Ceiling(count * .95) - 1],
        p99Ms = latencies[(int)Math.Ceiling(count * .99) - 1],
        sampledWorkerPeakRssBytes = sampler.PeakBytes,
        coordinatorAllocatedBytes = allocated,
        workerCount = shared.Snapshot.ReadyWorkers,
        correctness = "all tenant identities matched; CPU digests verified; all calls completed"
    };
}

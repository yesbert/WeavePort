...
internal static class LanguagePolicy
{
    internal static async Task RunAsync(Evidence evidence)
    {
        ...
        foreach (string language in new[] { "csharp", "python", "typescript" })
        {
            ...
            InvocationResult result = await session.InvokeAsync("echo", JsonSerializer.SerializeToElement(new { text = "synthetic" }));
            if (result.Status != "ok") throw new InvalidDataException("Language fixture failed startup.");
            ...
            if (evidence.Failures != before) throw new InvalidDataException("Language policy preflight failed.");
        }
    }

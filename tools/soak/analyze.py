...
def append_metric(lines, name, metrics):
...
def append_coordinator(lines, resources):
...
def main():
    ...
    if result["status"] == "running":
        print(
            "Run is still active. Inspect resources.jsonl and k6.log; k6 summary is not available yet."
        )
        return
    ...
    lines = [
        "# k6 soak result",
        "",
        f"Status: **{result['status']}**; stop reason: **{result.get('stopReason', 'running')}**.",
        "",
    ...
    for name in (
        ...
        append_metric(lines, name, metrics)
    resources = result.get("resources", {})
    lines += [
        "",
        f"Resource samples: {resources.get('samples', 0)}; peak summed RSS: {resources.get('peakSummedRssMiB', 0):.1f} MiB.",
        "",
    ...
    for i in range(result["settings"]["clients"]):
        ...
        def value(name, key):
            return metrics.get(name + suffix, {}).get("values", {}).get(key, 0)

        lines.append(
            f"| tenant-{i} | {value('completed_operations', 'count'):.0f} | {value('unexpected_errors', 'rate'):.8f} | {value('operation_ms', 'p(99)'):.2f} |"
        )
    append_coordinator(lines, resources)
    lines += [
        "",

...
The release version is `0.8.0`, licensed under [MIT](../LICENSE). All seven .NET packages and the exact compatibility matrix use 0.8.0; host API is 2, exclusive native protocol is 1 and concurrent native protocol is 2. Python/TypeScript SDK versions are 0.4.0; the qualified wheel and tarball accompany the GitHub release without claiming PyPI/npm registry publication. Historical internal distribution and measurement evidence retain their original identities. Public releases require complete clean candidate qualification and publication verification; see [current release status](status.md#release-080). A tag override cannot substitute for updating compatibility inputs.

The current [0.8.0 release report](../reports/release/0.8.0/README.md) records exact-tag qualification, the prepublication cancellation correction and verified public downloads. The previous [0.7.0 release report](../reports/release/0.7.0/README.md) records exact-tag qualification, the completed audit and verified public downloads. The [0.5.0 release report](../reports/release/0.5.0/README.md) records all seven public packages, tag qualification and download verification. The [0.4.0 release report](../reports/release/0.4.0/README.md) retains the exact qualification, artifact hashes and publication verification.
...
Exclusive streams hold an operation-wide worker lease, flush available items and use separate exchange/total deadlines. Composition.CollectAsync reads plugin-originated bytes in bounded blocks locally or through the Gateway. See [shared execution](shared-execution.md), [installed clients](installed-plugin-clients.md), [source sample](../examples/sources/README.md) and the [audit](../reports/release/0.6.0/audit.md). The [0.6.0 release report](../reports/release/0.6.0/README.md) records exact-tag qualification, the static audit, benchmark limits and verified publication evidence.

## 0.7.0 portable installations and consumer diagnostics

Hosting now seals deterministic schema-2 bundles through a public .NET API with an embedded compatibility policy. Runtime selection follows hashed .NET, Python and Node declarations; unchanged bundles can move between compatible runtime installations without executable-hash equality. Schema 1 and opt-in strict executable hashes retain their checks. Exact package versions and manifest pins still require deliberate offline migration.
...
## 0.8.0 failure contracts and code quality

Release 0.8.0 adds structured failure metadata (`PluginFailure`, `InvocationResult.Failure`, `PluginCallException.Failure`), stable exception codes and the opt-in `WorkerPoolOptions.DetailedFailureSink`. Existing .NET signatures remain available. Python and TypeScript author SDKs advance to 0.4.0; download their wheel/tarball from this release rather than reusing 0.3.0 artifacts. The internal reorganization covers runtime responsibilities, samples, tests and tooling without introducing a new runtime dependency.

### Migration from 0.7.0

- Upgrade all selected .NET packages together to 0.8.0 and author SDKs to the exact versions in the [compatibility matrix](package-compatibility.md). Rebuild/reseal installations offline; the new manifest identity requires deliberate application-pin migration. Keep original deployments for existing recovery pins. Do not edit a sealed manifest in place.
- Handle failures through `Failure?.Code ?? ErrorCode` with an unknown-code fallback. Gateway failures now use bounded platform/transport codes instead of gRPC detail text; unrecognized peer values become `unknown-error`. Callers matching previous text/status values must update their handling. Codes never grant permission to retry; retain `MayHaveExecuted` and reconcile uncertain effects.
- Review cancellation catches. A gateway `Cancelled` status produces `OperationCanceledException` only when the supplied local token is requested; otherwise it is a `PluginCallException` with code `cancelled`. Host cancellation attribution distinguishes caller cancellation, host shutdown, owned deadlines and unrelated internal cancellation.
- Execution and registered cleanup failures retain their causes; cleanup must not replace the primary failure. Python keeps `SessionCleanupError` and exposes its retained causes. Code that assumed only the last cleanup failure should inspect the documented metadata/aggregate causes.
- Default logs remain safe and source-generated. Event IDs 1001–1007 remain allocated; event 1008 reports diagnostic delivery failure. Invocation event 1003 includes code and cleanup outcome. Raw local exception details require an explicitly configured protected sink. Exception messages are not machine contracts.

Host API 2 and native protocols 1/2 remain unchanged. Gateway Reply fields 10–13 are additive; old peers may omit failure details. This does not qualify arbitrary mixed package families. See [failure codes](failure-codes.md), [runtime diagnostics](runtime-diagnostics.md) and the [release evidence](../reports/release/0.8.0/README.md). No new Docker containment or capacity/performance guarantee is introduced.

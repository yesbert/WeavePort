...
internal static class ConsumerFindingChecks
{
    internal static async Task RunAsync()
    {
        foreach (var policy in Enum.GetValues<WorkerReusePolicy>())
        {
            ...
            {
                type = "ready",
                protocol = policy == WorkerReusePolicy.Shared ? 2 : 1,
                pluginVersion = "1",
                concurrentCalls = 1,
                sessionCleanup = 1
            });
            try
            {
                WorkerEnvelope.ValidateReady(frame, "1.0.0", policy);
        ...
        try
        {
            var catalog = CreateDiscoveryFixture(root);
            foreach (var results in new[] { catalog.ListAll(), await catalog.ListAllAsync("test/v1") })
            {
                if (results.Count != 3 || results.Count(r => r.Installation is not null) != 1 || results.Count(r => r.Refusal is not null && r.Diagnostic is not null) != 2)
                {
                    throw new Exception("Incomplete discovery");
                }
            }
            if (catalog.ListAll("other").Any(r => r.Installation is not null))
            {
                throw new Exception("Contract mismatch hidden");
            }

            await CheckMalformedFrameworkAsync(root);
            using var cancelled = new CancellationTokenSource();
            cancelled.Cancel();
            try
            {
                await catalog.ListAllAsync(cancellationToken: cancelled.Token);
                throw new Exception("Cancellation ignored");
            }
            catch (OperationCanceledException) { }
            try
            {
                new InstalledPluginCatalog(Path.Combine(root, "missing"), new Dictionary<string, string>()).ListAll();
                throw new Exception("Root failure hidden");
            }
    ...
    private static async Task CheckMalformedFrameworkAsync(string root)
    {
        ...
        foreach (string content in new[] { "{}", new string(' ', 65537) })
        {
            ...
            await VerifyMalformedDiscoveryAsync(malformedFramework);

        }
    }
    private static async Task VerifyMalformedDiscoveryAsync(InstalledPluginCatalog malformedFramework)
    {
        foreach (var entries in new[] { malformedFramework.ListAll(), await malformedFramework.ListAllAsync() })
        {
            ...
            if (refusal.Refusal != "invalid-installation" || !refusal.Diagnostic!.Contains("Runtime 'dotnet' requires"))
            {
                throw new Exception("Malformed runtime interrupted discovery or lost runtime context.");
            }
        }
    }
    private static InstalledPluginCatalog CreateDiscoveryFixture(string root)
    {
        ...
        PluginInstallationSealer.Seal(new()
        {
            ...
            EntryPoints = new Dictionary<string, string> { ["dotnet"] = "worker.dll" }
        });
        ...
        return new InstalledPluginCatalog(root, new Dictionary<string, string> { ["dotnet"] = Environment.ProcessPath! });
    }

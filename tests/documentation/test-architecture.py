...
class ArchitectureTests(unittest.TestCase):
    ...
    def test_outward_dependency(self):
        self.change(
            "src/WeavePort.Abstractions/WeavePort.Abstractions.csproj",
            "</Project>",
            '<ItemGroup><ProjectReference Include="../WeavePort.Hosting/WeavePort.Hosting.csproj" /></ItemGroup></Project>',
        )
        self.assertTrue(
            any(
                "Outward dependency" in error for error in architecture.check(self.root)
            )
        )
    ...
    def test_facade_implementation(self):
        with (self.root / "sdks/typescript/src/index.ts").open("a") as output:
            output.write("\nclass Runtime {}\n")
        self.assertTrue(
            any("facade contains" in error for error in architecture.check(self.root))
        )

    def test_python_export_list_cannot_execute_implementation(self):
        path = self.root / "sdks/python/weaveport_sdk/__init__.py"
        with path.open("a") as output:
            output.write("\n__all__ = create_runtime()\n")
        self.assertTrue(
            any(
                "Python facade contains" in error
                for error in architecture.check(self.root)
            )
        )

    def test_scattered_version(self):
        self.change(
            "src/WeavePort.Hosting/WeavePort.Hosting.csproj",
            'Include="Tomlyn"',
            'Include="Tomlyn" Version="0.19.0"',
        )
        self.assertTrue(
            any("Scattered package" in error for error in architecture.check(self.root))
        )

    def test_duplicate_log_id(self):
        self.change(
            "src/WeavePort.Hosting/Diagnostics/RuntimeLogEvents.cs", "= 1002", "= 1001"
        )
        self.assertTrue(
            any("duplicate logging" in error for error in architecture.check(self.root))
        )

...
def main():
    ...
    assets = json.loads(
        (root / "src/WeavePort.Hosting/obj/project.assets.json").read_text()
    )
    actual = {
    ...
    if actual != expected:
        raise ValueError(
            "External dependency closure differs from reviewed net10.0 identities"
        )
    for name, version in expected.items():
        copy_package(root, assets, name, version)

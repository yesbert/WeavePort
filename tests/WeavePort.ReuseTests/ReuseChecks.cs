...
internal sealed class ReuseChecks
{
    private int _checks;
    ...
    internal async Task RunAsync(string[] args)
    {
        ...
        if (args.Length > 5)
        {
            profiles = new()
            {
                ["docker-" + args[5]] = new DockerProfile(args[6], MemoryMiB: 64, CpuCount: 1)
            };
        }
        foreach (var (language, original) in profiles)
        {
            await VerifyLanguageAsync(language, original);
        }
        if (args.Length <= 5)
        {
            await VerifyMalformedCleanupAsync();
        }
        Console.WriteLine($"PASS {_checks} reuse assertions");
    }
    private async Task VerifyLanguageAsync(string language, ExecutionProfile original)
    {
        foreach (var policy in new[] { WorkerReusePolicy.CustomerBound, WorkerReusePolicy.ApprovedSessions })
        {
            await VerifyPolicyAsync(language, original, policy);
        }
        await VerifySchedulerAsync(original);
        await VerifyLifecycleAsync(original);
    }
    private async Task VerifyPolicyAsync(string language, ExecutionProfile original, WorkerReusePolicy policy)
    {
        ...
        await using var a = await host.BindAsync(Context("a"), profile, _callbacks, ["who"]);
        await using var b = await host.BindAsync(Context("b"), profile, _callbacks, ["who"]);
        var first = await CallAsync(a, "session");
        CheckSession(first, "a");
        var second = await CallAsync(b, "session");
        CheckSession(second, "b");
        Assert((first.Instance == second.Instance) == (policy == WorkerReusePolicy.ApprovedSessions), language + " policy boundary");
        for (int i = 0; i < 10; i++)
        {
            CheckSession(await CallAsync(i % 2 == 0 ? a : b, "session"), i % 2 == 0 ? "a" : "b");
        }

        Assert(host.Snapshot.Workers == (policy == WorkerReusePolicy.ApprovedSessions ? 1 : 2), "pool size");
        await CallAsync(a, "stash");
        var probe = await CallAsync(b, "probe");
        ...
        var stream = await a.InvokeAsync("$sdk.start", JsonSerializer.SerializeToElement(new
        {
            operation = "rows",
            input = new
            {
        ...
        var during = await CallAsync(b, "session");
        CheckSession(during, "b");
        ...
        CheckSession(await CallAsync(a, "session"), "a");
        if (policy == WorkerReusePolicy.ApprovedSessions)
        {
            await VerifyFailedCleanupAsync(host, a, b);
        }
        Console.WriteLine($"{language} {policy}: passed");
    }
    private async Task VerifyFailedCleanupAsync(PluginHost host, IPluginSession a, IPluginSession b)
    {
        ...
        {
            var failed = await CallAsync(a, fault);
            Assert(failed.Status == (fault == "cleanup-hang" ? "timeout" : "failed"), "cleanup failure surfaced");
            var recovered = await CallAsync(b, "session");
            CheckSession(recovered, "b");
        ...
        var cancelled = await a.InvokeAsync("$sdk.call", JsonSerializer.SerializeToElement(new
        {
            operation = "delay",
            input = new
            {
        ...
        var afterCancel = await CallAsync(b, "session");
        CheckSession(afterCancel, "b");
        Assert(afterCancel.Instance != cancelled.Instance, "cancelled worker cannot be reused");
    }
    private async Task VerifyMalformedCleanupAsync()
    {
        foreach (string mode in new[] { "legacy", "missing", "invalid", "duplicate" })
        {
            ...
            await using var binding = await host.BindAsync(Context("raw"), profile, _callbacks, []);
            var result = await CallAsync(binding, "session");
            Assert(result.Status == "protocol-error" && host.Snapshot.Workers == 0, "missing/malformed cleanup protocol fails closed: " + mode);
        }
    }
    private async Task VerifySchedulerAsync(ExecutionProfile original)
    {
        ...
        try
        {
            for (int i = 0; i < 20; i++)
            {
                plugins.Add(await host.BindAsync(Context("scheduled-" + i), profile, _callbacks, ["who"]));
            }

            var ids = new HashSet<string>();
            foreach (var plugin in plugins)
            {
                var result = await plugin.InvokeAsync("$sdk.call", JsonSerializer.SerializeToElement(new
                {
                    operation = "session",
                    input = new
                    {
            ...
            {
                CheckSession(concurrent[i], plugins[i].Tenant);
            }
    ...
    private async Task VerifyLifecycleAsync(ExecutionProfile original)
    {
        var profile = original with
        {
            ReusePolicy = WorkerReusePolicy.ApprovedSessions
        ...
        {
            ReusableIdleTimeout = TimeSpan.FromMinutes(1)
        }, timeProvider: clock);
        await using var a = await host.BindAsync(Context("a"), profile, _callbacks, ["who"]);
        var first = await CallAsync(a, "session");
        CheckSession(first, "a");
        await a.DisposeAsync();
        await using var b = await host.BindAsync(Context("b") with
        {
            Plugin = "other-plugin"
        }, profile, _callbacks, ["who"]);
        var next = await CallAsync(b, "session");
        CheckSession(next, "b");
        ...
        var restarted = await CallAsync(b, "session");
        CheckSession(restarted, "b");
        ...
        await CallAsync(b, "stash");
        string approvedInstance = b.Instance;
        await using var bound = await host.BindAsync(Context("bound"), original, _callbacks, ["who"]);
        var probe = await CallAsync(bound, "probe");
    ...
    private void CheckSession(InvocationResult result, string tenant)
    {
    ...
    private static PluginContext Context(string tenant) => new(tenant, "plugin", "1", "default", JsonSerializer.SerializeToElement(new { secret = "secret-" + tenant }));
    private static Task<InvocationResult> CallAsync(IPluginSession session, string operation) => session.InvokeAsync("$sdk.call", JsonSerializer.SerializeToElement(new { operation, input = new { } }));
}

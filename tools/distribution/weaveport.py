...
def command(args, **kwargs):
    return subprocess.run(
        [str(arg) for arg in args], env=environment(), check=True, **kwargs
    )
...
def runtime_check(manifest, dotnet, python):
    ...
    runtimes = command(
        [dotnet, "--list-runtimes"], capture_output=True, text=True
    ).stdout
    if {
        line.split()[1]
        for line in runtimes.splitlines()
        if line.startswith("Microsoft.NETCore.App 10.0.")
    } != {"10.0.12"}:
...
def verify_app_part(root, expected, app, part):
...
def app_content(root, manifest):
    expected = {
        name.removeprefix("apps/"): sha
        for name, sha in manifest["files"].items()
        if name.startswith("apps/")
    }
    verify_files(root / "var", expected)
    for app, part in product(APPS, ("host", "releases")):
        verify_app_part(root, expected, app, part)
    python_root = root / "var/decision-room/python"
    release = read(root / "var/decision-room/releases/1/installation.json")
    sdk = {
        name.removeprefix("sdk/"): sha.lower()
        for name, sha in release["RuntimeFiles"].items()
...
def install(bundle, destination, dotnet, python):
    if destination.exists() or destination.is_symlink():
        raise ValueError(
            "Destination already exists; installation never replaces existing state"
        )
...
def doctor(root):
    ...
    if receipt["schema"] != 1 or receipt["root"] != str(root):
        raise ValueError(
            "Installation moved or receipt incompatible; install into a new destination"
        )
...
def main():
    ...
    setup = sub.add_parser(
        "install", help="Install this extracted bundle into a new directory"
    )
    ...
    run = sub.add_parser(
        "run", help="Run an installed example (use absolute paths for custom files)"
    )
    run.add_argument("app", choices=APPS)
    run.add_argument("arguments", nargs=argparse.REMAINDER)
    args = parser.parse_args()
    root = Path(__file__).resolve().parent
    if args.action == "install" and not args.dotnet:
        raise ValueError("dotnet prerequisite missing; pass --dotnet")
    if args.action == "install":
        install(
            root,
            args.destination.parent.resolve() / args.destination.name,
            args.dotnet.resolve(),
            args.python.resolve(),
        )
        return 0
    ...
    if args.action == "doctor":
        print(
            f"PASS: {receipt['version']} content and qualified runtimes; state preserved"
        )
        return 0
    ...
    os.execve(
        ...
        [
            receipt["dotnet"],
            str(root / "var" / args.app / "host" / f"{name}.Host.dll"),
            *arguments,
        ],
        env,
    )

...
internal static class OptionalVerification
{
    ...
    internal static async Task RunAsync(string[] args)
    {
        if (args[0] == "--api")
        {
            ApiChecks.Run(args[1], args.Contains("--candidate"));
            return;
        }
        ...
        async Task<LocalPluginClient> BindAsync(string tenant) => new(await host.BindAsync(
            new PluginContext(tenant, "optional-fixture", "1", "default", Check.Json(new
            {
        ...
        await VerifyTlsAsync(BindAsync, storage);
        await VerifyMalformedGatewayAsync();
    ...
    private static async Task VerifyTlsAsync(Func<string, Task<LocalPluginClient>> bindAsync, string storage)
    {
        await using (var tls = new TlsFixture())
        {
            ...
            int count = 0;
            await foreach (var number in remote.StreamAsync("numbers", Check.Json(33)))
            {
                Check.That(number.GetInt32() == count++, "TLS SDK stream ordering");
            }

            Check.That(count == 33, "TLS SDK complete stream");
            await foreach (var number in remote.StreamAsync("numbers", Check.Json(100)))
            {
                break;
            }
    ...
    private static async Task VerifyMalformedGatewayAsync()
    {
    ...
    private static async Task VerifyMappingAsync(IBoundPluginClient client, string storage, string label)
    {

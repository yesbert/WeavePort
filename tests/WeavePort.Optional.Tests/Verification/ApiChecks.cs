...
internal static class ApiChecks
{
    internal static void Run(string root, bool candidate)
    {
        ...
        {
            Check.That(assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()!.InformationalVersion.Split('+')[0] == "0.8.0", assembly.GetName().Name + " loaded version");
        ...
        if (candidate)
        {
            Console.WriteLine("Candidate only: API baseline was not accepted.");
            return;
        }
    ...
    private static void AddAssembly(List<string> lines, Assembly assembly)
    {
    ...
    private static void AddType(List<string> lines, Type type)
    {

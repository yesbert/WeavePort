...
**Internal candidate policy, reviewed 2026-09-12.** The [machine-readable matrix](../compatibility/local-v1.json) defines one exact combination. It is embedded in `WeavePort.Hosting` and consumed by the offline installation sealer. The current internal distribution uses this exact matrix; no general SemVer range is accepted.
...
| Core host packages | Abstractions, Hosting, Sdk.Client, each `0.1.0-internal.2` | Exact declaration plus actual packed/loaded metadata checks |
| C# author SDK (`dotnet`) | `WeavePort.Sdk` `0.1.0-internal.2` | Entry-specific declaration, packed metadata and native startup/call checks |
...
The metadata is a trusted deployment declaration, not runtime package attestation. The gate inspects real packed package metadata and loaded assembly versions; the installation resolver validates declarations and file hashes. It does not interrogate every transitive runtime dependency inside a running worker. Same `0.1.0-internal.2` labels across different development builds do not prove identical bytes or behavior. Keep one coherent tested deployment and its pinned content; stable-file requirements in [installed-plugin resolution](installed-plugins.md) still apply.

Old manifests without `Compatibility` fail closed. Rebuild/reseal only offline. This changes manifest identity, so existing pins require the original deployment or new application state; recovery never upgrades a pin automatically. Manifest schema remains 1 during this unreleased evolution. A released-format evolution will need an explicit migration/format decision rather than relying on this development convention.

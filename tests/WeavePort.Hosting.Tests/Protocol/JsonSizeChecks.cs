...
internal static class JsonSizeChecks
{
    internal static int Run()
    {
        ...
        var boundaries = from limit in new[] { 128 << 10, 256 << 10, 512 << 10 }
                         from delta in new[] { -1, 0, 1 }
                         select (limit, delta);
        foreach (var (limit, delta) in boundaries)
        {
            JsonElement value = JsonSerializer.SerializeToElement(new string('x', limit - 2 + delta));
            if (JsonSize.Measure(value) != limit + delta)
            {
                throw new Exception("Exact UTF-8 quota boundary changed.");
            }
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            Check(JsonSerializer.SerializeToElement(new
            {
                value,
                i,
                nested = new[] { value }
        ...
        Parallel.For(0, 100, i =>
        {
            ...
            {
                tenant = i,
                text = new string('x', i * 100)
            });
            if (JsonSize.Measure(value) != JsonSerializer.SerializeToUtf8Bytes(value).Length)
            {
                throw new Exception("Concurrent size ownership.");
            }
        });
        ...
        void Check(JsonElement value)
        {
            if (JsonSize.Measure(value) != JsonSerializer.SerializeToUtf8Bytes(value).Length)
            {
                throw new Exception("Serialized byte length changed.");
            }

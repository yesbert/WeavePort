...
internal static class DockerCommandChecks
{
    internal static async Task RunAsync()
    {
        ...
        {
            try { DockerCommand.ResolveExecutable(path); throw new Exception("Relative Docker executable accepted"); }
            catch (ArgumentException) { }
        }
        foreach (double cpu in new[] { 0, -1, double.NaN, double.PositiveInfinity })
        {
            try { await new DockerProfile("unused", CpuCount: cpu).ResolveAsync(default); throw new Exception("Invalid CPU limit accepted"); }
        ...
        try
        {
            ...
            try { DockerCommand.ResolveExecutable(executable); throw new Exception("Missing Docker executable accepted"); }

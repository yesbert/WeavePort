...
internal static class CatalogChecks
{
    ...
    internal static async Task RunAsync(string root, string python, string node)
    {
        ...
        await SealAsync(root, work, new("python-shared", "1", Contract, true), "python", python);
        await SealAsync(root, work, new("node-shared", "1", Contract, true), "node", node);
        ...
        {
            await RejectApprovalAsync(host, plugin, approval with
            {
                Ownership = WorkerReusePolicy.CustomerBound
            }, "CustomerBound approval cannot authorize Shared declaration.");
            await RejectApprovalAsync(host, plugin, approval with
            {
                Degree = 3
            }, "Degree must not exceed the installation declaration.");
            await using var shared = await host.ShareAsync(plugin, approval, JsonSerializer.SerializeToElement(new
            {
            }), new NoCallbacks(), []);
            await using var client = shared.For("catalog-tenant");
            JsonElement result = await client.CallAsync("echo", JsonSerializer.SerializeToElement(new
            {
                origin = plugin.Identity.Plugin
            }));
        ...
        await SealAsync(root, work, new("python-exclusive", "1", "catalog-serial/v1", false), "python", python);
        var exclusive = catalog.Resolve("python-exclusive", "1", "catalog-serial/v1");
        await using (var client = await host.BindAsync(exclusive, new PluginApproval { TrustedCode = true, MaximumMemoryMiB = 128 }, new TenantBinding("serial-tenant", JsonSerializer.SerializeToElement(new
        {
        })), new NoCallbacks(), []))
        {
            Require((await client.CallAsync("echo", JsonSerializer.SerializeToElement(new
            {
            }))).GetProperty("tenant").GetString() == "serial-tenant", "Installed serial launch failed.");
        }
        CheckRuntimeTampering(catalog, work);
        await SealAsync(root, work, new("python-mismatch", "2", "mismatch/v1", true), "python", python);
        InstalledPlugin mismatch = catalog.Resolve("python-mismatch", "2", "mismatch/v1");
        bool refused = false;
        try
        {
            await using var unexpected = await host.ShareAsync(mismatch, approval, JsonSerializer.SerializeToElement(new
            {
            }), new NoCallbacks(), []);
        }
    ...
    private static async Task RejectApprovalAsync(PluginHost host, InstalledPlugin plugin, PluginApproval approval, string label)
    {
        ...
        try
        {
            await using var unexpected = await host.ShareAsync(plugin, approval, JsonSerializer.SerializeToElement(new
            {
            }), new NoCallbacks(), []);
        }
    ...
    private static void CheckRuntimeTampering(InstalledPluginCatalog catalog, string work)
    {
        ...
        try
        {
            catalog.Resolve("python-shared", "1", Contract);
        }
    ...
    private sealed record InstallationDefinition(string Plugin, string Version, string Contract, bool Shared);

    private static async Task SealAsync(string root, string work, InstallationDefinition definition, string runtime, string executable)
    {
        var (plugin, version, contract, shared) = definition;
        ...
        string sdkEntry = runtime == "python"
            ? await CopyPythonSdkAsync(root, executable, sdk)
            : CopyNodeSdk(root, sdk);
        arguments.AddRange(["--sdk-path", sdkEntry]);
        if (!shared)
        {
            arguments.Add("--exclusive");
        }
        ...
        var manifest = new
        {
            ...
            Files = files,
            RuntimeFiles = new Dictionary<string, string> { [runtime] = Hash(executable) },
            ...
            Launch = new
            {
                ...
                Arguments = arguments,
                MemoryMiB = 128,
                Ownership = new[] { shared ? "Shared" : "CustomerBound" },
                MaximumDegree = shared ? 2 : 1
            }
        };
    ...
    private static async Task<string> CopyPythonSdkAsync(string root, string executable, string sdk)
    {
        ...
        foreach (string file in Directory.EnumerateFiles(package, "*.py"))
        {
    ...
    private static string CopyNodeSdk(string root, string sdk)
    {
        ...
        return Path.Combine(sdk, "index.js");
    }

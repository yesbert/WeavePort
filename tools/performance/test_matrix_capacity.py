...
class CapacityTests(unittest.TestCase):
    def evaluate(self, backlogs, peaks=None, passed=True):
        with tempfile.TemporaryDirectory() as folder:
            rows = [
                {
                    ...
                    "counts": {
                        "offered": 1000 + i * 1000,
                        "completed": 1000 + i * 1000 - pending,
                        "dropped": 0,
                    },
                    "windowResponseMs": {
                        "count": 1000,
                        "p99": peaks[i] if peaks else 10,
                    },
                }
            ...
            return stability(
                path,
                {
                    "config": {"seconds": 60, "workers": 8, "rate": 500},
                    "meetsOneSecondSlo": passed,
                },
            )
    ...
    def test_queue_growth_rejects_good_aggregate_latency(self):
        self.assertFalse(
            self.evaluate([0, 0, 0, 30, 60, 100, 200, 300, 400])["qualified"]
        )

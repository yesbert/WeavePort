"""Measure the current delivered core through one multilingual SDK benchmark suite."""
import argparse
import hashlib
import json
import os
        ...
        with zipfile.ZipFile(feed / filename) as archive:
            for entry in archive.namelist():
                if not entry.startswith('lib/net10.0/') or not entry.endswith('.dll'):
                    continue
                folder = 'sdk-csharp' if filename.startswith('WeavePort.Sdk.0') else 'sdk-worker-host'
                path = ROOT / 'artifacts' / folder / Path(entry).name
                checksum = hashlib.sha256(archive.read(entry)).hexdigest()
                if digest(path) != checksum:
                    raise ValueError('Loaded implementation differs from qualified package: ' + path.name)
...
def main():
    ...
    parser.add_argument('--distribution', type=Path, required=True, help='Extracted bundle or installed payload directory')
    args = parser.parse_args()
    output = ROOT / 'artifacts/performance' / (time.strftime('%Y%m%d-%H%M%S') + '-' + uuid.uuid4().hex[:8])
    ...
    record = {'status': 'running', 'sourceCommit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
              'coreVersion': '0.1.0-internal.2', 'packages': expected, 'stages': []}
    def stage(name, command):
        print('STAGE: ' + name, flush=True)
        started = time.monotonic()
        with (output / (name + '.log')).open('w') as log:
            result = subprocess.run(command, cwd=ROOT, env=env, stdout=log, stderr=subprocess.STDOUT)
        record['stages'].append({'name': name, 'exitCode': result.returncode, 'seconds': round(time.monotonic() - started, 3)})
    ...
    try:
        ...
        record['optionalGateway'] = digest(ROOT / 'artifacts/sdk-worker-host/WeavePort.Sdk.Gateway.dll')
        record['runtimeExecutables'] = {name: digest(Path(shutil.which(name)).resolve()) for name in ('dotnet', 'python3', 'node')}
        stage('correctness', ['./scripts/sdk.sh', str(output / 'correctness'), 'verify'])
        stage('benchmarkdotnet', ['./scripts/sdk.sh', str(output / 'timing'), '--filter', '*', '--artifacts', str(output / 'bdn')])
            ...
            for topology in ('local', 'gateway'):
                for scenario in ('callback', 'list-64k'):
                    for repeat in (1, 2):
                        ...
                        stage(name, ['./scripts/sdk.sh', str(output / name), 'load', language, topology, scenario, '16', str(repeat), '3'])
        verify_packages(feed, expected)
        assert record['loadedCore'] == verify_loaded(feed, expected)
        assert record['loadedAuthors'] == verify_authors(ROOT, args.distribution.resolve())

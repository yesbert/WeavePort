...
internal sealed class CapacityDiagnostics : IDisposable
{
    ...
    {
        if (_enabled) AppDomain.CurrentDomain.FirstChanceException += OnException;
    }

    private void OnException(object? sender, FirstChanceExceptionEventArgs args)
    {
        Exception error = args.Exception;
        if (error is not (InvalidDataException or JsonException or InvalidOperationException or KeyNotFoundException)) return;
        if (Interlocked.Increment(ref _count) > 100) return;
        // Synthetic-fixture diagnostic only. No payloads or exception messages are retained.
        _errors.Enqueue(new { type = error.GetType().Name, stack = error.StackTrace });
    }

    internal Task SaveAsync(string output) => File.WriteAllTextAsync(Path.Combine(output, "exceptions.json"),
        JsonSerializer.Serialize(new { enabled = _enabled, observed = _count, errors = _errors }, new JsonSerializerOptions { WriteIndented = true }));
    ...
    internal static async Task<double?> SwapMiBAsync()
    {
        ...
        return amount[^1] switch { 'M' => number, 'G' => number * 1024, 'K' => number / 1024, _ => throw new InvalidDataException("Unknown swap unit") };
    }
}

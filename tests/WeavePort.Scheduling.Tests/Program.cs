...
if (args.Contains("--worker"))
{
    ...
    if (args.Contains("--stderr-flood"))
    {
        ...
        for (int line = 0; line < 1000; line++) Console.Error.WriteLine(new string('y', 1000));
    }
...
internal static class SchedulingChecks
{
    ...
    private static readonly ProcessProfile Profile = new(Environment.ProcessPath!, [typeof(SchedulingChecks).Assembly.Location, "--worker"],
        trustedCode: true, workspaceRoot: Root, reservedMemoryMiB: 64) { Reconstructible = true };
    ...
    private static async Task<ScheduledPlugin> Bind(PluginHost host, string tenant, string plugin = "one", IHostCallbacks? callbacks = null,
        PluginWorkClass workClass = PluginWorkClass.Normal) => (ScheduledPlugin)await host.BindAsync(new(tenant, plugin, "1", "test", Empty), Profile with { WorkClass = workClass },
    ...
    private static async Task DormantRegistry()
    {
        ...
        await Until(() => host.Scheduling!.Active == 0);
    ...
    {
        await using var host = new PluginHost(Options() with { MaximumQueuedCallsPerTenant = 1, QueueTimeout = TimeSpan.FromMilliseconds(200) });
        var holds = new Holds();
    ...
    {
        await using var host = new PluginHost(Options(1) with { NormalTimeout = TimeSpan.FromMilliseconds(500) });
        var a = await Bind(host, "a");
    ...
    {
        await using var host = new PluginHost(Options() with { MemoryBudgetMiB = 128, IdleTimeout = TimeSpan.FromMilliseconds(200) });
        var holds = new Holds();
    ...
    private static async Task CleanupFailure()
    {
        ...
        await Until(() => host.Snapshot.Tenants == 1);

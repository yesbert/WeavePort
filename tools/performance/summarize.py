...
def collect(run):
    ...
    if provenance['status'] != 'passed' or any(s['exitCode'] for s in provenance['stages']):
    ...
    for file in sorted((run / 'bdn/results').glob('*-report-full.json')):
        ...
        for row in report['Benchmarks']:
            ...
            timing.append(dict(kind='cold' if 'Cold' in row['Type'] else 'warm',
                               parameters=dict(parse_qsl(row['Parameters'])),
                               statisticsNs=row['Statistics'], memory=row.get('Memory'),
                               measurements=[m for m in row['Measurements'] if m['IterationMode'] == 'Workload'
                                             and m['IterationStage'] in ('Actual', 'Result')]))
    warm = {tuple(r['parameters'].get(k) for k in ('Language', 'Topology', 'Case')) for r in timing if r['kind'] == 'warm'}
    cold = {tuple(r['parameters'].get(k) for k in ('Language', 'Topology')) for r in timing if r['kind'] == 'cold'}
    languages, topologies = ('csharp', 'python', 'typescript'), ('local', 'gateway')
    if warm != {(a, b, c) for a in languages for b in topologies for c in ('tiny', 'callback', 'list-64k', 'list-8m')} or cold != {(a, b) for a in languages for b in topologies} or len(timing) != 30:
        raise ValueError('Incomplete or duplicate benchmark matrix')
    loads = [read(p) for p in sorted(run.glob('load-*/load.json'))]
    expected = {(a, b, c, d) for a in languages for b in topologies for c in ('callback', 'list-64k') for d in (1, 2)}
    ...
    for row in loads:
        if row['errors'] or row['concurrency'] != 16 or len(row['lanes']) != 16 or any(l['Errors'] or not l['Count'] for l in row['lanes']):
    ...
    provenance.update(fixtureHashes=fixtures, runId=run.name,
...
def render(data):
    ...
    lines = ['# Current performance baseline — ' + p['runId'][:8], '',
    ...
    for language in ('csharp', 'python', 'typescript'):
        for topology in ('local', 'gateway'):
            rows = [r for r in data['timing'] if r['parameters']['Language'] == language and r['parameters']['Topology'] == topology]
            values = {r['parameters'].get('Case', 'cold'): r['statisticsNs']['Mean'] / 1e6 for r in rows}
            lines.append('| ' + ' | '.join([language, topology] + [f'{values[k]:.3f}' for k in ('tiny', 'callback', 'list-64k', 'list-8m', 'cold')]) + ' |')
    lines += ['', '## Individual requests and resources', '',
    ...
    for language in ('csharp', 'python', 'typescript'):
        for topology in ('local', 'gateway'):
            for scenario in ('callback', 'list-64k'):
                rows = [r for r in data['loads'] if (r['language'], r['topology'], r['scenario']) == (language, topology, scenario)]
                ranges = []
                for key, divisor in (('rps', 1), ('p99Ms', 1), ('peakCombinedRss', 1048576)):
                    values = [r[key] / divisor for r in rows]
                    ranges.append(f'{min(values):.2f}–{max(values):.2f}')
                lines.append('| ' + ' | '.join([language, topology, scenario] + ranges) + ' |')
    lines += ['', 'RSS samples cover the measuring host, optional gateway and known worker root processes every 100 ms. Shared pages can be counted repeatedly; descendants, Docker VM and unrelated applications are excluded, and sampling can miss peaks. Managed allocations cover the measuring .NET process only. Raw per-client outcomes and resource samples are retained in evidence.json.', '',

...
{
    throw new ArgumentException("New evidence directory required.");
}

if (TestProfiles.SocketTransport is not null)
{
    throw new NotSupportedException("This bounded runner qualifies Docker CLI transport only.");
}
...
try
{
    ...
    {
        await Pressure.RunAsync(evidence, repeat);
    }
}
finally
{
    ...
    await evidence.CheckAsync("Pre-existing containers remain running", () =>
    {
        if (before.Split('\n', StringSplitOptions.RemoveEmptyEntries).Except(after.Split('\n')).Any())
        {
            throw new InvalidOperationException("Pre-existing container changed state.");
        }

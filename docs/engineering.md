...
Run `dotnet run --project tools/WeavePort.CodeStyle -c Release -- "$PWD"` to verify maintained C# in `src`, `samples`, `examples`, `plugins`, `tests`, `benchmarks` and `tools`. Add `--write` to apply the SDK Roslyn formatter and required braces. For a focused review, pass one or more `--area=relative-directory` arguments; they replace the default area list and report the same checks. The gate rejects drift, methods over 60 code lines and files over 350 lines. It uses the formatting workspace shipped with the pinned SDK and preserves intentional line wrapping; syntax-tree normalization is not a source formatter. Generated `.g.cs` files are verified through their generators instead. Keep simple auto-properties compact while using braces for control-flow bodies. `.editorconfig` mirrors the editor conventions. This covers methods and constructors; primary record constructors with many fields are reviewed as meaningful configuration or result objects, not arbitrarily wrapped. Scenario methods keep setup and assertions together within those bounds.
...
The C# readability gate now also rejects forbidden control flow. Run `python3 scripts/check-python-control-flow.py` for maintained Python sources and `npm run check:control-flow --prefix sdks/typescript` after `npm ci --prefix sdks/typescript` for maintained TypeScript and JavaScript sources. All three checks execute positive and negative syntax fixtures before scanning maintained sources. The isolated candidate workflow runs them in CI. TypeScript uses the AST API of the existing pinned compiler as development tooling; its unstable API must be rechecked deliberately when upgrading TypeScript. No parser is shipped with the SDK.

The Python and TypeScript/JavaScript gates inspect versioned and non-ignored new source files, including test scenarios and tooling. Historical `reports/` source snapshots and archived OpenSpec artifacts are excluded: their original bytes are evidence. Dependency caches and generated build output are ignored; C# generated contracts retain their generator check. Review still owns naming, cohesion, duplication, dependency direction and meaningful method boundaries; a passing syntax metric is not proof of every clean-code property.

...
internal static class RuntimeRequirementChecks
{
    internal static void Run()
    {
        ...
        foreach (var (policy, expected) in new[] { ("Disable", (string?)null), ("LatestPatch", "10.0.1"), ("Minor", "10.0.1"), ("LatestMinor", "10.4.5"), ("Major", "10.0.1"), ("LatestMajor", "11.0.6") })
        {
            ...
            {
                throw new Exception($".NET selection {policy}: {selected} != {expected}");
            }
        }
        if (DotnetRequirements.Select(new("core", "10.0.2", "LatestPatch"), [new(10, 0, 1)]) is not null)
        {
            throw new Exception("Minimum patch bypass");
        }

        if (DotnetRequirements.Select(new("core", "10.0.0", "Minor"), [new(11, 0, 0)]) is not null)
        {
            throw new Exception("Major bypass");
        }

        if (DotnetRequirements.Select(new("core", "10.0.0", "Major"), [new(11, 0, 0)]) is null)
        {
            throw new Exception("Major denied");
        }
        ...
        if (DotnetFrameworks.Matches(multi, "Microsoft.NETCore.App 10.0.12 [/nonexistent]"))
        {
            throw new Exception("Missing framework accepted");
        }

        foreach (string language in new[] { "python", "node" })
        {
            VerifyOracle(language);
        }
        Console.WriteLine("PASS ecosystem version and framework selection cases");
    }
    private static void VerifyOracle(string language)
    {
        ...
        foreach (var item in fixture.RootElement.EnumerateArray())
        {
            ...
            if (actual != expected)
            {
                throw new Exception($"{language} oracle mismatch: {range} / {version}: {actual} expected {expected}");
            }
        }
    }
    private static void VerifyPythonRanges()
    {
        ...
        {
            if (PythonSpecifier.Matches(range, version) != expected)
            {
                throw new Exception($"Python mismatch {range} / {version}");
            }
        }
        foreach (string bad in new[] { "", "3.11", ">=3.*", "~=3", "==3.11rc1.*", ">3.11+abc" })
        {
            try
            {
                PythonSpecifier.Parse(bad);
                throw new Exception("Invalid Python range accepted " + bad);
            }

...
spec = importlib.util.spec_from_file_location('receiver', Path(__file__).resolve().parents[2] / 'scripts/ci/receive-site.py')
...
def archive(extra=None, run=1):
    ...
    with tarfile.open(fileobj=buffer, mode='w:gz') as tar:
        files = {name: b'text' for name in ('index.html', 'llms.txt', 'llms-full.txt', 'docs/ai-documentation.md')}
        files['deployment.json'] = json.dumps({'revision': 'a' * 40, 'run': run}).encode()
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_path_links_and_server_config_are_rejected(self):
        for name, kind in [('../escape.txt', tarfile.REGTYPE), ('link.md', tarfile.SYMTYPE),
                           ('.htaccess', tarfile.REGTYPE), ('execute.php', tarfile.REGTYPE)]:

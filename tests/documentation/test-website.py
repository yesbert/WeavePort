...
class WebsiteTests(unittest.TestCase):
    def test_nested_navigation_preserves_anchor(self):
        result = website.rewrite_href('samples/DecisionRoom/README.md', 'docs/examples/DecisionRoom.md',
                                      '../../docs/plugin-sdk.md#python', website.sources())
    ...
    def test_logo_stays_local_and_source_code_goes_to_repository(self):
        ...
        self.assertEqual(website.rewrite_href('website/index.md', 'index.md', 'assets/logo.png', mapping),
                         'assets/logo.png')
        result = website.rewrite_href('docs/plugin-sdk.md', 'docs/plugin-sdk.md',
                                      '../examples/sdk/python/plugin.py', mapping)
    ...
    def test_missing_source_is_not_silently_published(self):
        with self.assertRaises(ValueError):
            website.rewrite_href('website/index.md', 'index.md', 'missing.md', website.sources())
    ...
    def test_source_edits_change_full_reference(self):
        ...
        def changed(path, *args, **kwargs):
            ...
            return content + '\nFreshness regression sentinel\n' if path == ROOT / 'docs/status.md' else content
    ...
    def test_python_example_matches_the_maintained_provider(self):
        ...
        function = next(node for node in ast.parse(code).body if isinstance(node, ast.AsyncFunctionDef))
        fixture = ast.parse((ROOT / 'examples/sdk/python/plugin.py').read_text())
        expected = next(node for node in fixture.body if isinstance(node, ast.AsyncFunctionDef) and node.name == 'echo')
    ...
    def test_other_language_examples_use_existing_echo_handlers(self):
        ...
        self.assertTrue(all(line.strip() in fixture for line in code.splitlines() if line.strip()))
        code = re.search(r'```csharp\n(.*?)```', page, re.S)[1]
        handler = code[code.index('.Function'):code.index('.RunAsync')]
        compact = lambda text: re.sub(r'\s+', '', text)
        self.assertIn(compact(handler), compact((ROOT / 'examples/sdk/csharp/Program.cs').read_text()))

...
def verify_templates(root, target, run):
    ...
    for name, _ in APPS.values():
        for role in ("Host", "Plugin" if name == "DecisionRoom" else "Worker"):
            run(f"build-{name}-{role}", ["dotnet", "build", f"samples/{name}/{role}",
                "-c", "Release", "--nologo", "-p:NuGetAudit=false"], cwd=consumer)
    for assets in consumer.glob("samples/**/obj/project.assets.json"):
        data = read(assets)
        assert all(Path(path).is_relative_to(consumer) for path in data["packageFolders"])
        sdk_feed = Path(shutil.which("dotnet")).resolve().parent / "library-packs"
        assert all(Path(path).is_relative_to(consumer) or Path(path) == sdk_feed
                   for path in data["project"]["restore"]["sources"])
    expected_packages = read(target / "payload/distribution.json")["components"]["nuget"]
...
def verify_refusals(bundle, target, setup, run):
    run("wrong-runtime", [*setup, target, "--dotnet", sys.executable], 1, "runtime executable mismatch")
...
def verify(archive):
    ...
    def run(label, args, expected=0, contains=None, cwd=root):
        completed = subprocess.run([str(arg) for arg in args], cwd=cwd, env=environment(),
                                   capture_output=True, text=True, timeout=180)
        output = completed.stdout + completed.stderr
        (logs / f"{label}.log").write_text(output)
        assertions = sum(line.startswith("PASS:") for line in output.splitlines())
        stage = {"name": label, "exitCode": completed.returncode, "expectedExitCode": expected,
    ...
    try:
        ...
        receipt.unlink()
        run("incomplete-installation", [*launcher, "run", "appointment-desk"], 1, "Incomplete installation")
        receipt.write_bytes(receipt_bytes)
        changed = target / "var/appointment-desk/host/AppointmentDesk.Host.dll"
        original = changed.read_bytes()
        changed.write_bytes(original + b"changed")
        run("damaged-installed-host", [*launcher, "run", "appointment-desk"], 1, "Content mismatch")
        ...
        before = {p.relative_to(target / "var").as_posix(): digest(p)
                  for p in (target / "var").rglob("*") if p.is_file() and not p.is_symlink()}
        run("used-destination-refused", [*setup, target], 1, "Destination already exists")
        run("doctor-after-use", [*launcher, "doctor"])
        after = {p.relative_to(target / "var").as_posix(): digest(p)
                 for p in (target / "var").rglob("*") if p.is_file() and not p.is_symlink()}
        assert before == after, "Reinstall/doctor changed application state"
        verify_templates(root, target, run)
        result["applicationAssertions"] = sum(stage["assertions"] for stage in result["stages"]

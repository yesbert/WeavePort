...
Frames are limited to 1 MiB of UTF-8 and parsed to depth 32. Worker envelopes reject duplicate reserved fields (`type`, `id`, `callbackId`, `operation`, `payload`, `value`, `protocol`, `pluginVersion`, `code`), including escaped spellings. Ready protocol versions must be the supported integer; invalid numeric conversion becomes a protocol error. Nested payload/result JSON retains application semantics. Exclusive bindings run one invocation at a time under configured queued or fail-fast admission. The default callback budget is eight per internal invocation and is configurable. The host deadline includes startup, transport and callback waits. Callback grants and immutable tenant identity constrain authority; traces are retained. Nested plugin calls from callback scopes are denied before dispatch.
...
Shared SDK workers advertise `protocol: 2` and `concurrentCalls: 1`. This deliberately causes older protocol-1 hosts to refuse them. The host sends `configure` with the approved degree before dispatch. One reader routes result and callback frames by invocation ID, and a serialized writer prevents frame interleaving. Unknown IDs or malformed frames retire the channel. SDK streams and binary sources are exclusive-only.

Cancellation returns promptly and revokes callback authority. Its slot and context remain owned until a terminal acknowledgement or bounded retirement; cancellation is not evidence that arbitrary plugin code stopped. Process failure fails outstanding invocations without replay. Restart budgets and silence/cancellation limits belong to operator approval.

Result streams may return an empty unfinished batch as a heartbeat. The producer retains a pending iterator advancement rather than restarting it for each heartbeat. Binary sources use `$sdk.source.open`, `$sdk.source.read` and `$sdk.source.close`, with 4–256 KiB decoded blocks transported as bounded base64 JSON. The client keeps one operation lease through cleanup.

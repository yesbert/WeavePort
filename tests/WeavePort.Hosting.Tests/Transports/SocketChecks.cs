...
internal static class SocketChecks
{
    internal static async Task<int> RunAsync()
    {
        if (!OperatingSystem.IsLinux())
        {
            try
            {
                new UnixSocketTransport("/ipc", "/ipc").Validate();
            }
        ...
        foreach (string path in new[] { "relative", "/bad,path", "/bad\npath", "/bad\0path", "/" + new string('x', 100) })
        {
            try
            {
                new UnixSocketTransport(path, "/daemon").Validate();
                throw new Exception("Invalid path accepted");
            }
        ...
        try
        {
            using (var listener = new WorkerSocket(transport, "waiting"))
            {
                using (var deadline = new CancellationTokenSource(TimeSpan.FromMilliseconds(50)))
                {
                    try
                    {
                        ...
                        throw new Exception("Missing connection did not time out");
                    }
            ...
            await CheckConnectionAsync(transport, root);
            checks++;

            if (Directory.GetFileSystemEntries(root).Length != 0)
            {
                throw new Exception("Socket leaf leaked");
            }

            File.SetUnixFileMode(root, UnixFileMode.UserRead | UnixFileMode.UserWrite | UnixFileMode.UserExecute | UnixFileMode.OtherRead);
            try
            {
                using var rejected = new WorkerSocket(transport, "exposed");
                throw new Exception("Shared root accepted");
            }
    ...
    private static async Task CheckConnectionAsync(UnixSocketTransport transport, string root)
    {
        using (var listener = new WorkerSocket(transport, "connected"))
        {
            using (var client = new Socket(AddressFamily.Unix, SocketType.Stream, ProtocolType.Unspecified))
            {
                using (var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(2)))
                {
                    ...
                    if (!listener.Running)
                    {
                        throw new Exception("Accepted connection not live");
                    }

                    client.Dispose();
                    if (await listener.Stream.ReadAsync(new byte[1], deadline.Token) != 0 || listener.Running)
                    {
                        throw new Exception("Peer exit not observed");
                    }

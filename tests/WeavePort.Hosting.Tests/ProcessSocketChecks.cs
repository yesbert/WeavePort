...
internal static class ProcessSocketChecks
{
    internal static async Task<int> RunAsync()
    {
        ...
        if (File.GetUnixFileMode(endpoint.Path) != (UnixFileMode.UserRead | UnixFileMode.UserWrite)) throw new Exception("Private socket mode");
        checks += 2;
        using var cancel = new CancellationTokenSource(TimeSpan.FromMilliseconds(30));
        try { await endpoint.AcceptAsync(cancel.Token); throw new Exception("Expected cancelled accept"); }
        catch (OperationCanceledException) { checks++; }
        string marker = Path.Combine(directory, "cleanup-obstruction");
        await File.WriteAllTextAsync(marker, "synthetic");
        try { endpoint.Dispose(); throw new Exception("Uncertain cleanup must fail"); }
        catch (IOException) { checks++; }
        File.Delete(marker);
        endpoint.Dispose();
        if (Directory.Exists(directory)) throw new Exception("Endpoint retained after cleanup retry");
        checks++;
        ...
        if (buffered.Stream!.Socket.SendBufferSize < 262144 || buffered.Stream.Socket.ReceiveBufferSize < 262144) throw new Exception("Host buffer request not effective");
        return checks + 1;
    }
}

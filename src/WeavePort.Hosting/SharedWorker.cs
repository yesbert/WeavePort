...
internal sealed partial class SharedWorker(SharedPlugin plugin, WorkerPool pool, TimeProvider clock) : IAsyncDisposable
{
    ...
    internal async Task StartAsync(CancellationToken token)
    {
        ...
        await SendAsync(new { type = "configure", degree = plugin.Options.Degree }, token);
        lock (_sync)
        {
    ...
    internal async Task<WeavePort.Abstractions.InvocationResult> InvokeAsync(SharedCall call)
    {
        ...
        catch (OperationCanceledException) when (call.Token.IsCancellationRequested)
        {
            ...
            return call.Result(call.CallerToken.IsCancellationRequested ? "cancelled" : "timeout", Instance, Empty);
        }
    ...
    private async Task CancelAsync(SharedCall call, Worker worker)
    {
        try
        {
            ...
            await SendAsync(new { type = "cancel", id = call.Id }, deadline.Token, worker);

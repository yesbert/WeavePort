...
internal sealed class LinuxComparison(string mode, string imagePrefix, string socketDirectory)
{
    ...
    {
        if (!OperatingSystem.IsLinux()) throw new PlatformNotSupportedException("This comparison requires Linux");
        string mode = Environment.GetEnvironmentVariable("WEAVEPORT_COMPARE_MODE") ?? "process";
        if (mode is not ("process" or "container")) throw new ArgumentException("Unknown comparison mode");
        if (!config.UseUnixSocket || config.SocketBufferBytes is not null) throw new ArgumentException("Use OS-default Unix socket buffers for both comparison modes");
        ...
        {
            mode, topology = "Linux Docker Desktop VM; same trusted coordinator container; child processes versus separate worker containers; NOT bare metal",
            os = RuntimeInformation.OSDescription, runtime = Environment.Version.ToString(), architecture = RuntimeInformation.ProcessArchitecture.ToString(),
            hostingSha256 = Hash(typeof(PluginHost).Assembly.Location), runnerSha256 = Hash(typeof(LinuxComparison).Assembly.Location),
            csharpSha256 = Hash(config.Csharp), pythonSha256 = Hash(config.PythonScript), typescriptSha256 = Hash(config.TypeScriptScript),

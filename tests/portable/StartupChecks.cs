...
internal static class StartupChecks
{
    internal static async Task RunAsync(string work, string node)
    {
        ...
        {
            await VerifyOwnershipAsync(work, node, ownership);
        }
    }

    [UnsupportedOSPlatform("windows")]
    private static async Task VerifyOwnershipAsync(string work, string node, WorkerReusePolicy ownership)
    {
        ...
        {
            throw new Exception("Incompatible initial runtime executed plugin");
        }

        File.WriteAllText(state, "v24.0.0\n");
        await AttemptAsync(installation, ownership, marker, true, state);
        if (File.ReadAllLines(marker).Length != 1)
        {
            throw new Exception("Incompatible replacement executed plugin");
        }
    ...
    private static async Task AttemptAsync(InstalledPlugin installation, WorkerReusePolicy ownership, string marker, bool replace, string state)
    {
        ...
        try
        {
            ...
            {
                shared = await host.ShareAsync(installation, approval, JsonSerializer.SerializeToElement(new
                {
                }), new NoCallbacks(), []);
            ...
            {
                client = await host.BindAsync(installation, approval, new("tenant", JsonSerializer.SerializeToElement(new
                {
                })), new NoCallbacks(), []);
            }
            ...
            if (replace)
            {
                await CrashAfterRuntimeChangeAsync(client, marker, state);
            }
            try
            {
                await client.CallAsync("echo", JsonSerializer.SerializeToElement(1));
                throw new Exception("Expected runtime refusal");
            }
    ...
    private static async Task CrashAfterRuntimeChangeAsync(IBoundPluginClient client, string marker, string state)
    {
        await client.CallAsync("echo", JsonSerializer.SerializeToElement(1));
        if (!File.Exists(marker))
        {
            throw new Exception("Initial process did not start");
        }

        File.WriteAllText(state, "v99.0.0\n");
        try
        {
            await client.CallAsync("crash", JsonSerializer.SerializeToElement(1));
        }
        catch (PluginCallException) { }
    }

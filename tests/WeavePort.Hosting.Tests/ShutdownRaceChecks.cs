...
internal static class ShutdownRaceChecks
{
    internal static async Task RunAsync()
    {
        ...
        try { await start; throw new Exception("Worker was published after shutdown"); }
        ...
        try { await pool.AcquireAsync(profile, "1", "b", default); throw new Exception("Shutdown accepted a new worker"); }
    ...
    private sealed class DelayedWorker(DelayedProfile profile, string version) : Worker(profile, version)
    {
        ...
        {
            if (!profile.Started) throw new Exception("Cleanup raced startup");
            profile.Destroyed++;

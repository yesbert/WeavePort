...
Candidate `82d8890` passed 3,530 reported assertions and froze 364 files. PR #34 passed all required checks and merged as `aa69112c2e351c0de800808566022773b3813136`. On this exact revision, main CI (35651608863), CodeQL (35651608551) and SonarQube (35651608629) passed. The complete SonarQube export reports zero open issues, zero unreviewed hotspots, zero duplication, 85.6% new-code coverage and 83.7% overall coverage. All nine capability deltas match their maintained specifications. Annotated tag `v0.6.0` selects this revision; publication evidence is recorded separately after the protected release workflow completes.

Timing evidence retains the original measured source/assembly identities. Later shutdown, callback-retention and static-analysis corrections passed functional qualification but were not substituted into those historical measurements or presented as new timing runs.

## Publication verification

Protected release workflow 35652454003 qualified and published annotated tag v0.6.0 successfully. The exact tag passed 3,530 reported assertions and froze 364 files. Before approving the existing environment, all seven packages, seven symbol packages, Python wheel and TypeScript tarball matched their exact qualified manifest entries. No environment protection was changed. All 16 public GitHub artifacts match the original bytes, and every original ZIP entry in all seven indexed NuGet downloads matches, allowing only the repository signature added by NuGet. The release report links durable evidence and distinct original/public hashes.

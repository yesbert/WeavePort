...
class HeadroomControls(unittest.IsolatedAsyncioTestCase):
    async def rejected(self, workers, free_percent, expected):
        with tempfile.TemporaryDirectory(prefix='wp-matrix-headroom-') as directory:
            ...
            with patch('reuse_matrix.pressure', return_value={'freePercent': free_percent, 'swapUsedMiB': 0}), \
                 patch('reuse_matrix.docker_memory.snapshot', return_value={'allContainersRawMiB': 0}), \
                 patch('reuse_matrix.command', new=AsyncMock(return_value='')), \
                 patch('reuse_matrix.EngineLane.start', new_callable=AsyncMock) as start:
                with self.assertRaisesRegex(RuntimeError, 'Stage failed'):
                    await measure(folder, {'mode': 'trusted', 'transport': 'engine', 'workers': workers,
                                          'memory': 64, 'seconds': 1}, 'unused-image', engine)

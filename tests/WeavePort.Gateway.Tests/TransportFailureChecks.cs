...
internal static class TransportFailureChecks
{
    internal static void Run()
    {
        ...
        var cases = new Dictionary<StatusCode, string>
        {
            ...
            [StatusCode.PermissionDenied] = "denied",
            [StatusCode.Unauthenticated] = "binding-denied",
            [StatusCode.ResourceExhausted] = "resource-exhausted",
            [StatusCode.Internal] = "gateway-failed",
        ...
        foreach (var (status, code) in cases)
        {
            ...
            if (actual.ErrorCode != code || actual.Message.Contains("sensitive"))
            {
                throw new Exception("Transport code depends on detail text.");
            }
        }

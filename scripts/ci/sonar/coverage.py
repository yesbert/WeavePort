...
SUITES = (
    ...
    "WeavePort.Scheduling.Tests",
    "WeavePort.ReuseTests",
    "WeavePort.Optional.Tests",
    "WeavePort.Shared.Tests",
    "WeavePort.ConcurrentSdkTests",
)
...
def run_suite(command, log, timeout=180):
    ...
    with subprocess.Popen(
        ...
        try:
            ...
            print(
                f"FAIL: suite exceeded the {timeout}-second watchdog; stopping its owned process group.",
                file=log,
                flush=True,
            )
            try:
...
def suite_command(name, mode):
    command = [
        "dotnet",
        str(ROOT / "tests" / name / "bin/Debug/net10.0" / (name + ".dll")),
    ]
...
def execute():
    ...
    for name, mode in cases:
        ...
        with (OUTPUT / (label + ".log")).open("w") as log:
            command = suite_command(name, mode)
    ...
    with (OUTPUT / "concurrent-sdk-wire.log").open("w") as log:
        wire_exit = run_suite(
            [sys.executable, "tests/WeavePort.ConcurrentSdkTests/check.py"], log
        )
...
def collect():
    ...
    subprocess.run(
        ["npm", "ci", "--prefix", "sdks/typescript", "--ignore-scripts"], check=True
    )
    ...
    subprocess.run(
        [
            "dotnet",
            "publish",
            "examples/gateway/Worker",
            "-c",
            "Debug",
            "-p:UsePackedCore=false",
            "-o",
            "artifacts/optional/worker",
            "--nologo",
        ],
        check=True,
    )
    ...
    os.environ["WP_SHARED_CSHARP_FIXTURE"] = str(
        ROOT
        / "tests/WeavePort.ConcurrentSdkTests/bin/Debug/net10.0/WeavePort.ConcurrentSdkTests.dll"
    )
    ...
    hosting = [
        ...
        if module.get("name") == "WeavePort.Hosting.dll"
    ]
    if (
        covered == 0
        or not hosting
        or not any(int(module.get("lines_covered", "0")) > 0 for module in hosting)
    ):

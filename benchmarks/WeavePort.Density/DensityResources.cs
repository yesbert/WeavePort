...
internal sealed partial class DensityResources(DensityConfig config, CancellationTokenSource stop, List<IPluginSession> clients, Func<WorkerPoolSnapshot> snapshot)
{
    ...
    internal object Summary() => new { peakHostMiB = _peakHost, peakChildRssMiB = _peakChildren, peakCombinedRssMiB = _peakCombined, samples = _samples, peakWorkers = _peakWorkers,
    ...
    internal async Task RunAsync()
    {
        ...
        try
        {
            while (!stop.IsCancellationRequested)
            {
                ...
                info.ArgumentList.Add("-axo"); info.ArgumentList.Add("pid=,ppid=,rss=");
                using var ps = Process.Start(info)!;
                string lines = await ps.StandardOutput.ReadToEndAsync();
                await ps.WaitForExitAsync();
                if (ps.ExitCode != 0) throw new IOException("RSS observer failed");
                var rows = lines.Split('\n', StringSplitOptions.RemoveEmptyEntries).Select(line => line.Split(' ', StringSplitOptions.RemoveEmptyEntries))
                    .Select(parts => (Pid: int.Parse(parts[0]), Parent: int.Parse(parts[1]), Rss: double.Parse(parts[2]) / 1024)).ToArray();
                var owned = new HashSet<int> { Environment.ProcessId };
                bool added;
                do { added = false; foreach (var row in rows) if (row.Pid != ps.Id && owned.Contains(row.Parent)) added |= owned.Add(row.Pid); } while (added);
                ...
                _peakHost = Math.Max(_peakHost, host); _peakChildren = Math.Max(_peakChildren, children); _peakCombined = Math.Max(_peakCombined, host + children); _samples++;
                string[] dockerInstances = config.Adapter == "docker" ? await ReadDockerInstancesAsync(owned) : [];
                await output.WriteLineAsync(JsonSerializer.Serialize(new { at = DateTimeOffset.UtcNow, hostMiB = host, childrenMiB = children, runtime,
                ...
                { Failure = "resource-or-operator-stop"; stop.Cancel(); break; }

    ...
    internal static async Task RunAsync()
    {
        ...
        var schedulerThread = new Thread(() =>
        {
            lock (schedulerSync)
            {
                ...
                if (entered) Monitor.Exit(hostSync);
                observed.TrySetResult(entered);
            }
        }) { IsBackground = true };
        schedulerThread.Start();
        if (!schedulerHeld.Wait(TimeSpan.FromSeconds(3))) throw new Exception("Scheduler lock fixture failed.");
        ...
        if (!hostAvailable) throw new Exception("Host shutdown held the host lock while waiting for the scheduler lock.");

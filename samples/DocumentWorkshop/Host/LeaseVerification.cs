...
internal static class LeaseVerification
{
    internal static async Task RunAsync(string root, CancellationToken token)
    {
        ...
        HostCall Call(ReadRequest request, string tenant = "tenant-a", string profile = "profile-a") => new(new PluginContext(tenant, "plain", "1", profile, JsonSerializer.SerializeToElement(new { })), "invocation", "document.read", JsonSerializer.SerializeToElement(request, Json.Options), "trace");

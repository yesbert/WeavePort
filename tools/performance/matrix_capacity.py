...
from reuse import command, write
...
def stability(folder, result):
    ...
    samples = [row for row in samples if 0 < row['elapsed'] <= result['config']['seconds']]
    if len(samples) < 6:
    ...
    allowed = max(2 * result['config']['workers'], result['config']['rate'] * .1)
    windows = [row['windowResponseMs']['p99'] for row in samples if row.get('windowResponseMs', {}).get('count', 0) >= 100]
    value = {'earlyMeanBacklog': early, 'lateMeanBacklog': late, 'allowedBacklogGrowth': allowed,
             'maxWindowP99Ms': max(windows, default=None)}
    value['qualified'] = result['meetsOneSecondSlo'] and late - early <= allowed and bool(windows) and max(windows) <= 1000
    return value
...
async def main():
    ...
    parser.add_argument('--mode', required=True, choices=['trusted', 'forkserver', 'process'])
    ...
    if not 0 < args.low < args.high or not 1 <= args.workers <= 32 or not 30 <= args.seconds <= 300 or not 0 <= args.steps <= 4:
    ...
    identity.update({'image': image, 'matrixHashes': {str(p.relative_to(root)): hashlib.sha256(p.read_bytes()).hexdigest()
                     for p in list((root / 'tools/performance').glob('*matrix*.py')) + list((root / 'benchmarks/WeavePort.Reuse/fixture').glob('*.py'))}})
    write(args.output / 'identity.json', identity)
    engine, results = docker_memory.Engine(), []
    async def point(rate):
        qualified = True
        for repeat in [1, 2]:
            config = dict(mode=args.mode, workers=args.workers, seconds=args.seconds, cpus='1', transport='engine',
                          arrival='poisson', rate=rate, seed=1728 + repeat, repeat=repeat)
    ...
    if not await point(low):
        write(args.output / 'bracket.json', {'qualifiedLow': None, 'firstRejected': low, 'reason': 'Lower point did not qualify; reduce offered load'})
        return
    if await point(high):
        write(args.output / 'bracket.json', {'qualifiedLow': high, 'firstRejected': None, 'reason': 'No upper boundary found in the configured bracket'})
        return
    for _ in range(args.steps):
        midpoint = (low + high) // 2
        if midpoint == low:
            break
        if await point(midpoint):
            low = midpoint
        else:
            high = midpoint
    write(args.output / 'bracket.json', {'qualifiedLow': low, 'firstRejected': high,

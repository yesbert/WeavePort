...
class Histogram:
    ...
    def add(self, value):
        value = max(0.0, value)
        key = math.ceil(math.log(max(0.001, value), 1.01))
    ...
    def report(self):
        ...
        for name, fraction in [("p50", 0.5), ("p95", 0.95), ("p99", 0.99)]:
            result[name] = self.percentile(fraction)
        return result
    ...
    def percentile(self, fraction):
        ...
        for key, occurrences in sorted(self.buckets.items()):
            ...
            if count < target:
                continue
            return min(self.maximum, 1.01**key)
        raise RuntimeError("Histogram count differs from its buckets")

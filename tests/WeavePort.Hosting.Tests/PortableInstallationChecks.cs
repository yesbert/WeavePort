...
internal static class PortableInstallationChecks
{
    ...
    private static async Task CheckAsync(string root)
    {
        ...
        if (identity.Digest != Convert.ToHexString(SHA256.HashData(bytes))) throw new Exception("Identity isn't manifest hash");
        if (PluginInstallationSealer.Seal(options) != identity) throw new Exception("Non-deterministic manifest");
        string moved = Path.Combine(root, "moved", "1");
        Directory.CreateDirectory(moved);
        foreach (string file in Directory.GetFiles(release)) File.Copy(file, Path.Combine(moved, Path.GetFileName(file)));
        if (PluginInstallationSealer.Seal(options with { ReleaseDirectory = moved }) != identity) throw new Exception("Path bound manifest");
        ...
        if (catalog.Resolve("test", "1", "test/v1", identity).Identity != identity || (await catalog.ResolveAsync("test", "1", "test/v1", identity)).Identity != identity) throw new Exception("Portable resolve");
        await RejectAsync(() => new InstalledPluginCatalog(root, new Dictionary<string, string>()).ResolveAsync("test", "1", "test/v1"));
        using (var cancelled = new CancellationTokenSource())
        {
            cancelled.Cancel();
            try { await catalog.ResolveAsync("test", "1", "test/v1", cancellationToken: cancelled.Token); throw new Exception("Cancellation ignored"); }
        ...
        if ((await independent.ResolveAsync("test", "1", "test/v1", expectedPin)).Identity != expectedPin) throw new Exception("Independent fixture pin changed");
        ...
        if (File.ReadAllText(selector) != "previous") throw new Exception("Refused activation changed selector");
        await CheckDeclarationsAsync(root);
        if (!OperatingSystem.IsWindows()) await CheckProbeAsync(root);
    }
    ...
    private static async Task CheckDeclarationsAsync(string root)
    {
        ...
        try { PluginInstallationSealer.Seal(options); throw new Exception("Invalid declaration accepted"); } catch (InvalidDataException) { }
        if (!before.SequenceEqual(File.ReadAllBytes(Path.Combine(release, "installation.json")))) throw new Exception("Failed seal mutated manifest");
        foreach (string declaration in new[] { "[project]\n", "[project]\ndynamic = ['requires-python']\n", "[project]\nrequires-python = '>=3.11'\ndynamic = ['requires-python']\n", "[project\n" })
        {
            File.WriteAllText(Path.Combine(release, "pyproject.toml"), declaration);
            try { PluginInstallationSealer.Seal(options); throw new Exception("Invalid Python metadata accepted"); } catch (InvalidDataException) { }
        }
        ...
        {
            File.WriteAllText(Path.Combine(release, "package.json"), JsonSerializer.Serialize(new { engines = new { node = range } }));
            PluginInstallationSealer.Seal(options with { EntryPoints = new Dictionary<string, string> { ["node"] = "entry.mjs" }, Launch = new() { Runtime = "node" } });
        }
    ...
    private static async Task CheckProbeAsync(string root)
    {
        ...
        await RejectAsync(() => RuntimeProbe.RunAsync(script, "node", default));
        if (clock.Elapsed > TimeSpan.FromSeconds(10)) throw new Exception("Probe bound");
    }

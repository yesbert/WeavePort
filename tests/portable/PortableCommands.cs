...
internal static class PortableCommands
{
    internal static async Task RunAsync(string[] args)
    {
        ...
        clock.Restart();
        await using IBoundPluginClient client = await host.BindAsync(installation, new()
        {
            TrustedCode = true
        }, new("tenant", JsonSerializer.SerializeToElement(new
        {
        })), new NoCallbacks(), []);
        JsonElement answer = await client.CallAsync("echo", JsonSerializer.SerializeToElement(new
        {
            message = "portable"
        }));
        if (answer.GetProperty("message").GetString() != "portable")
        {
        ...
        Console.WriteLine(JsonSerializer.Serialize(new
        {
            ...
            runtimeSha256 = Convert.ToHexString(SHA256.HashData(runtime)),
            resolveMs,
            startAndCallMs = clock.Elapsed.TotalMilliseconds
        }));
    }
    private static void SealRelease(string root, string release)
    {
        Directory.CreateDirectory(release);
        foreach (string path in Directory.EnumerateFiles(AppContext.BaseDirectory))
        {
            File.Copy(path, Path.Combine(release, Path.GetFileName(path)), true);
        }

...
class DensityControls(unittest.TestCase):
    ...
    def test_missing_live_memory_fails(self):
        engine = FakeEngine(
            {
                "/containers/a/stats?stream=false&one-shot=true": {},
    ...
    def test_missing_exited_memory_is_explicit(self):
        engine = FakeEngine(
            {
                "/containers/a/stats?stream=false&one-shot=true": None,
                "/containers/a/json": None,
            }
        )
        ...
        engine = FakeEngine(
            {
                ...
                "/containers/a/stats?stream=false&one-shot=true": {
                    "memory_stats": {
                        "usage": 20 * 1048576,
                        "stats": {"inactive_file": 4 * 1048576},
                    }
                },
                "/containers/b/stats?stream=false&one-shot=true": {
                    "memory_stats": {"usage": 1024 * 1048576}
                },
            }
        )
    ...
    def test_cgroup_v1_cache_and_invalid_values(self):
        engine = FakeEngine(
            {
                "/containers/a/stats?stream=false&one-shot=true": {
                    "memory_stats": {
                        "usage": 100,
                        "stats": {"total_inactive_file": 20, "inactive_file": 80},
                    }
                }
            }
        )
        ...
        engine.responses["/containers/a/stats?stream=false&one-shot=true"][
            "memory_stats"
        ]["usage"] = -1
        with self.assertRaises(ValueError):
            docker_memory.memory_row(engine, {"Id": "a", "Names": []})
    ...
    def test_engine_timeout_is_not_zero_memory(self):
        with patch.object(
            FakeEngine, "get", side_effect=TimeoutError("engine timeout")
        ):

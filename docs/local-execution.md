...
- Pristine profiles are compared by frozen launch choices and version. Equivalent profiles share the reserve; customer-bound used roots and workspaces are never assigned to another customer. Explicit approved-session reuse keeps the same process/workspace and relies on registered cleanup; it remains trusted same-user execution. Idle release/restart creates fresh state.

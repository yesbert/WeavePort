...
internal static class FragmentChecks
{
    internal static async Task RunAsync()
    {
        ...
        if ((await frames.ReadAsync(default)).GetInt32() != 42)
        {
            throw new Exception("Prefix frame.");
        }

        var large = await frames.ReadAsync(default);
        if (large.GetString()!.Length != Frames.MaximumBytes - 2)
        {
            throw new Exception("Fragmented maximum frame.");
        }

        if ((await frames.ReadAsync(default)).GetInt32() != 17)
        {
            throw new Exception("Following frame.");
        }

        frames.Dispose();
        if (large.GetString()![^1] != 'x')
        {
            throw new Exception("Owned frame after disposal.");
        }

...
internal static class CompatibilityFixture
{
    internal static string Resolve(string root, string language, string runtime, string original, string version)
    {
        ...
        else File.Copy(original, Path.Combine(directory, Path.GetFileName(original)), true);
        ...
        var manifest = new
        {
            Schema = 1, Plugin = "sdk-echo", Version = version, Contract = "echo/v1",
            EntryPoints = new Dictionary<string, string> { [alias] = Path.GetFileName(original) },
            Files = files, RuntimeFiles = new Dictionary<string, string> { [alias] = Hash(runtime) }, Compatibility = policy
        };

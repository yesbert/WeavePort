...
def worker(connection, request):
    ...
    try:
        ...
        result = (
            {"status": "ok", "reusable": True} if request is None else invoke(request)
        )
        ...
        if request and request.get("exitDelayMs"):
            time.sleep(
                request["exitDelayMs"] / 1000
            )  # Controlled exit/reap deadline fixture.
    finally:
        connection.close()
...
class ForkBoundary:
    def __init__(self):
        ...
        multiprocessing.set_forkserver_preload(
            ["fork_boundary", "hashlib", "logging", "contextvars"]
        )
        self.context = multiprocessing.get_context("forkserver")
        warmup = self._run(
            None
        )  # Launch pristine template before any customer payload exists.
    ...
    def _run(self, request):
        ...
        value = (
            result[0]
            if result and not reader.is_alive()
            else {"status": "failed", "reusable": False, "error": "timeout"}
        )
        if (
            not isinstance(value, dict)
            or value.get("status") not in ("ok", "failed")
            or type(value.get("reusable")) is not bool
        ):
            value = {
                "status": "failed",
                "reusable": False,
                "error": "invalid-child-schema",
            }
        if exitcode != 0:
    ...
    def invoke(self, request):
        ...
        result["untrustedDeserializeMarkerPresent"] = Path(
            "/tmp/parent-deserialized-marker"
        ).exists()
        wipe()
        return result

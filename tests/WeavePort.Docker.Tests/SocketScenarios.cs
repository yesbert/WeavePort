...
internal static class SocketScenarios
{
    ...
    {
        if (TestProfiles.SocketTransport is not { } transport)
        {
            return;
        }
        ...
        {
            await evidence.CheckAsync(language + ": private read-only socket mount and endpoint cleanup", () => VerifyEndpointOwnershipAsync(language, transport));
        }
        await evidence.CheckAsync("socket: transport selection separates pristine workers", VerifyTransportPoolKeysAsync);
        await evidence.CheckAsync("socket: invalid mount fails without endpoint leak", () => VerifyInvalidMountAsync(transport));
    }

    private static Task<IPluginSession> BindAsync(PluginHost host, string tenant, DockerProfile profile) => host.BindAsync(
        new PluginContext(tenant, "demo", "1", "default", JsonSerializer.SerializeToElement(new
        {
    ...
    private static async Task VerifyEndpointOwnershipAsync(string language, UnixSocketTransport transport)
    {
        ...
        if (entries.Length != 1 || entries[0].GetProperty("Destination").GetString() != "/run/weaveport" || entries[0].GetProperty("RW").GetBoolean() ||
            entries[0].GetProperty("Source").GetString() != Path.Combine(transport.DockerDirectory, a.Instance))
        {
            throw new Exception("Unexpected endpoint exposure");
        }
        ...
        if (Directory.Exists(Path.Combine(transport.LocalDirectory, old)))
        {
            throw new Exception("Endpoint not removed");
        }

        await InvokeAsync(b);

    }
    private static async Task VerifyTransportPoolKeysAsync()
    {
        ...
        if (mounts.RootElement.GetArrayLength() != 1 || host.Snapshot.Pristine != 1)
        {
            throw new Exception("Transport pool keys mixed");
        }

    }
    private static async Task VerifyInvalidMountAsync(UnixSocketTransport transport)
    {
        ...
        {
            SocketTransport = transport with
            {
                DockerDirectory = "/weaveport-deliberately-missing"
            }
        };

...
def artifact_destination(source):
    parts = Path(source).parts
    if parts[1] in APPS and parts[2] in ("host", "releases"):
        return Path("apps", *parts[1:])
    if parts[1] == "packages":
        return Path("packages/nuget", parts[-1])
    if source == "artifacts/decision-room/wheel/weaveport_sdk-0.1.0-py3-none-any.whl":
        return Path("packages/python", parts[-1])
    if source == "artifacts/sdk-version-tests/weaveport-sdk-0.1.0.tgz":
        return Path("packages/typescript", parts[-1])
    return None
...
def copy_qualified_artifacts(checkout, bundle, expected):
    ...
    for source, sha in expected.items():
        target = artifact_destination(source)
        if target is None:
            continue
...
def copy_templates(checkout, bundle, result):
    ...
    for name in tracked:
        if name not in ("Directory.Build.props", "global.json") and not any(
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(
            subprocess.check_output(["git", "show", f"{commit}:{name}"], cwd=REPOSITORY)
        )
    ...
    (bundle / "templates/NuGet.Config").write_text(
...
def package(candidate, output):
    ...
    if (
        ...
        or digest(EVIDENCE / "artifact-manifest.json")
    ...
    manifest = {
        "schema": 1,
        "version": VERSION,
        "qualifiedSourceCommit": commit,
        "components": {
            "nuget": read(EVIDENCE / "package-set.json"),
            "python": "weaveport-sdk 0.1.0",
            "typescript": "@weaveport/sdk 0.1.0",
        },
        "prerequisites": {
            key: result[key]
            for key in ("platform", "architecture", "python", "runtimeExecutables")
        },
        "qualificationAssertions": result["assertions"],
        "qualifiedArtifacts": artifact_map,
        ...
        json.dumps(
            {
                ...
                "qualifiedArtifacts": len(artifact_map),
            },
            indent=2,
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "candidate", type=Path, help="Retained candidate directory containing checkout/"
    )

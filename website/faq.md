...
Start here if you are deciding how to make your .NET backend extensible. These answers describe the current 0.3.0 release and link to the detailed contracts.
...
The four public core NuGet packages are released under the [MIT license](../LICENSE). The current version is 0.3.0. Review [packages and compatibility](packages.md) before adopting or upgrading the evolving API.
...
Yes, if it offers the supported local stdio tools subset. Hosting 0.3.0 can discover and call tools using MCP 2025-11-25 or 2026-07-28, with the same lifecycle and tenant budgets as native plugins. You deploy the trusted server and select the protocol explicitly. One server may offer several tools; no AI model is required. See [MCP tools](../docs/mcp-plugins.md).

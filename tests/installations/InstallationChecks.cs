...
internal sealed class InstallationChecks
{
    ...
    private readonly InstalledPluginCatalog _catalog;
    private int _checks;

    internal InstallationChecks(string root, string dotnet)
    {
        _root = root;
        _dotnet = dotnet;
        _work = Path.Combine(root, "artifacts", "installation-tests", Guid.NewGuid().ToString("N"));
        _releases = Path.Combine(_work, "releases");
        foreach (string version in new[] { "1", "2" })
        {
            CopyRelease(Path.Combine(root, "artifacts/document-workshop/releases", version), Path.Combine(_releases, version));
        }
        _catalog = new InstalledPluginCatalog(_releases, new Dictionary<string, string> { ["dotnet"] = dotnet });
        _selector = Path.Combine(_work, "active.txt");
        _manifest = Path.Combine(_releases, "1", "installation.json");
        _baseline = File.ReadAllText(_manifest);
    }
    ...
    internal async Task RunAsync()
    {
        ...
        VerifyDiscovery(original.Identity);
        VerifySelectionAndBundle(original);
        VerifyCompatibility();
        VerifyManifestAndContent(original);
        await VerifyStartupVersionAsync();
        await VerifyApprovalAsync();
        await MixedLaunchChecks.RunAsync(_root);
        Console.WriteLine($"Verification passed: {_checks} assertions. Evidence: {_work}");
        File.WriteAllText(Path.Combine(_work, "result.txt"), $"{_checks} assertions passed\n");
    }
    ...
    private void Refused(Action action, string label)
    {
        try
        {
            action();
    ...
    private InstalledPlugin Resolve(string version = "1", InstallationIdentity? pin = null) => _catalog.Resolve("document-workshop", version, "document-workshop/v1", pin);
    private void Mutate(Action<JsonNode> edit, string label)
    {
        var document = JsonNode.Parse(_baseline)!;
        edit(document);
        File.WriteAllText(_manifest, document.ToJsonString());
        try
        {
            Refused(() => Resolve(), label);
        }
        finally { File.WriteAllText(_manifest, _baseline); }
    }
    private async Task RejectApprovalAsync(PluginHost approvalHost, InstalledPlugin declared, PluginApproval approval, string label)
    {
        try
        {
            await using var unused = await approvalHost.BindAsync(declared, approval,
                new TenantBinding("test", JsonSerializer.SerializeToElement(new
                {
                })), new Callbacks(), []);
        }
    ...
    private void VerifyDiscovery(InstallationIdentity originalIdentity)
    {
        var expandedRuntimes = new InstalledPluginCatalog(_releases, new Dictionary<string, string> { ["dotnet"] = _dotnet, ["unused-runtime"] = _dotnet });
        Check(expandedRuntimes.Resolve("document-workshop", "1", "document-workshop/v1").Identity == originalIdentity, "declared runtime subset accepts extra operator-approved aliases");
        string catalogRoot = Path.Combine(_work, "catalog");
        string nestedRelease = Path.Combine(catalogRoot, "document-workshop", "releases", "1");
        Directory.CreateDirectory(nestedRelease);
        foreach (string file in Directory.GetFiles(Path.Combine(_releases, "1")))
        {
            File.Copy(file, Path.Combine(nestedRelease, Path.GetFileName(file)));
        }

        File.WriteAllText(Path.Combine(catalogRoot, "document-workshop", "active.txt"), "1");
        var discovery = new InstalledPluginCatalog(catalogRoot, new Dictionary<string, string> { ["dotnet"] = _dotnet });
        Check(discovery.List("document-workshop/v1").Single().Identity == originalIdentity, "contract listing returns verified selected release");
        Check(discovery.List("different/v1").Count == 0, "contract listing excludes other contracts");
        Check(discovery.Resolve("document-workshop", "1", "document-workshop/v1", originalIdentity).Identity == originalIdentity, "nested catalog preserves digest pins");
    }
    private void VerifySelectionAndBundle(InstalledPlugin original)
    {
        string extra = Path.Combine(_releases, "1", "injected.dll");
        ...
        _catalog.Activate(_selector, "document-workshop", "2", "document-workshop/v1");
        Check(InstalledPluginCatalog.ReadSelection(_selector) == "2" && Resolve("1", original.Identity).Identity == original.Identity, "activation leaves exact pin resolvable");
        Refused(() => Resolve("../2"), "path traversal release rejected");
        Refused(() => Resolve("3"), "missing release has no fallback");
        Refused(() => _catalog.Resolve("other", "1", "document-workshop/v1"), "wrong plugin identity rejected");
        Refused(() => _catalog.Resolve("document-workshop", "1", "document-workshop/v2"), "unsupported domain contract rejected");
    }
    private void VerifyCompatibility()
    {
        ...
        File.WriteAllText(_manifest, _baseline.Replace("\"HostApi\": 2", "\"HostApi\": 2, \"HostApi\": 2"));
        Refused(() => Resolve(), "duplicate compatibility fields rejected");
        File.WriteAllText(_manifest, _baseline);
        var incompatible = JsonNode.Parse(_baseline)!;
        incompatible["Compatibility"]!["Protocol"] = 99;
        File.WriteAllText(_manifest, incompatible.ToJsonString());
        string oldSelector = File.ReadAllText(_selector);
        Refused(() => _catalog.Activate(_selector, "document-workshop", "1", "document-workshop/v1"), "incompatible activation rejected");
        Check(File.ReadAllText(_selector) == oldSelector, "refused compatibility activation preserves existing default");
        File.WriteAllText(_manifest, _baseline);
    }
    private void VerifyManifestAndContent(InstalledPlugin original)
    {
        ...
        File.WriteAllText(_manifest, _baseline.Replace("\"Schema\": 2", "\"Schema\": 2, \"Schema\": 2"));
    ...
    private async Task VerifyStartupVersionAsync()
    {
        string binary = Resolve().EntryPoints["dotnet"];
        byte[] saved = File.ReadAllBytes(binary);
        // Trusted metadata deliberately describes v1 content as v2 to reach the independent startup guard.
        string secondManifest = Path.Combine(_releases, "2", "installation.json");
        var second = JsonNode.Parse(File.ReadAllText(secondManifest))!;
        string secondBinary = Path.Combine(_releases, "2", "DocumentWorkshop.Worker.dll");
        ...
        await using (var host = new PluginHost(options: new WorkerPoolOptions(MaximumPristineWorkers: 0)))
        {
            var session = await host.BindAsync(new PluginContext("test", "markdown", "2", "default", JsonSerializer.SerializeToElement(new
            {
                ...
                new ProcessProfile(_dotnet, [wrong.EntryPoints["dotnet"], "markdown"], true, Path.Combine(_work, "workers")), new Callbacks(), []);
    ...
    private async Task VerifyApprovalAsync()
    {
        await using (var approvalHost = new PluginHost())
        {
            var declared = Resolve();
            await RejectApprovalAsync(approvalHost, declared, new PluginApproval(), "untrusted approval refused before startup");
            await RejectApprovalAsync(approvalHost, declared, new PluginApproval { TrustedCode = true, MaximumCallbacks = 0 }, "callback budget validated before startup");
            await RejectApprovalAsync(approvalHost, declared, new PluginApproval { TrustedCode = true, StartupTimeout = TimeSpan.Zero }, "startup deadline validated before startup");
            await RejectApprovalAsync(approvalHost, declared, new PluginApproval { TrustedCode = true, MaximumMemoryMiB = 64 }, "memory approval must cover launch reservation");
        }
    }
}

...
internal sealed partial class SharedWorker(SharedPlugin plugin, WorkerPool pool, TimeProvider clock) : IAsyncDisposable
{
    ...
    private async Task<WeavePort.Abstractions.InvocationResult> InvokeCoreAsync(SharedCall call)
    {
        ...
        try
        {
            ...
            await SendAsync(new InvokeFrame(FrameKinds.Invoke, call.Id, call.Operation, call.Payload, call.Context, call.Id), call.Token, worker);
        ...
        catch (OperationCanceledException) when (call.Token.IsCancellationRequested)
        {
            ...
            return call.Result(CancellationCode(call), Instance, Empty);
        }
    ...
    private string CancellationCode(SharedCall call)
    {
        if (call.CallerToken.IsCancellationRequested)
        {
            return FailureCodes.Cancelled;
        }

        return plugin.Lifetime.IsCancellationRequested ? FailureCodes.Disabled : FailureCodes.Timeout;
    }
    ...
    private async Task CancelAsync(SharedCall call, Worker worker)
    {
        try
        {
            ...
            await SendAsync(new
            {
                type = FrameKinds.Cancel,
                id = call.Id
            }, deadline.Token, worker);

...
WeavePort is an embedded backend plugin platform for owner-controlled applications and plugins. Version 0.7.0 targets .NET 10 and is licensed under MIT. The API is evolving; review the exact compatibility matrix before upgrading.
...
The qualified .NET package set is `0.7.0`; host API is level 2, with protocol 1 for exclusive and protocol 2 for shared workers. Python and TypeScript author SDKs use version `0.3.0`. Do not mix binaries from different deliveries merely because an API or protocol number matches. Exact package hashes and qualified runtimes are recorded in the release qualification evidence.

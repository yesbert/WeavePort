...
public static class Workload
{
    ...
    public static async Task<JsonElement[]> ExecuteAsync(IPluginClient client, string name, CancellationToken token = default)
    {
        if (name == "tiny")
        {
            return [await client.CallAsync("echo", JsonSerializer.SerializeToElement(new { value = "sdk" }), token)];
        }

        if (name == "callback")
        {
            return [await client.CallAsync("owner", JsonSerializer.SerializeToElement(new { operation = "host.owner", input = new { tenant = "forged" } }), token)];
        }

        var items = new List<JsonElement>();
        await foreach (JsonElement item in client.StreamAsync("records", Query(name == "list-64k" ? 8 : 1024, 8192), token))
        {
            items.Add(item);
        }
    ...
    public static void Check(JsonElement[] output, string name, string tenant)
    {
        ...
        if (name == "callback")
        {
            CheckOwner(output, tenant);
            return;
        }
        int count = name == "list-64k" ? 8 : 1024;
        if (output.Length != count)
        {
            throw new InvalidDataException("Record count.");
        }

        for (int i = 0; i < output.Length; i++)
        {
            CheckRow(output[i], i, 8192, tenant);
        }
    }
    private static void CheckEcho(JsonElement[] output)
    {
        if (output.Length != 1 || output[0].GetProperty("value").GetString() != "sdk")
        {
            throw new InvalidDataException("Echo mismatch.");
        }
    }

    private static void CheckOwner(JsonElement[] output, string tenant)
    {
        if (output.Length != 1 || output[0].GetProperty("owner").GetString() != tenant ||
            output[0].GetProperty("secret").GetString() != "synthetic-" + tenant)
        {
            throw new InvalidDataException("Callback authority mismatch.");
        }
    }

...
import sys
import time
sdk_path = sys.argv[sys.argv.index("--sdk-path") + 1] if "--sdk-path" in sys.argv else str(Path(__file__).resolve().parents[3] / "sdks/python")
if sdk_path != "-":
...
def cpu(value, context):
    result = hashlib.pbkdf2_hmac("sha256", b"weaveport", b"benchmark", value.get("iterations", 100000))
    return dict(tenant=context.tenant, digest=result.hex())

@app.function("malformed")
async def malformed(value, context):
    sys.__stdout__.write("{not-json}\n" if value.get("invalidJson") else '{"type":"result","id":"unknown","value":null}\n')

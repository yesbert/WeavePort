...
internal static class BoundedStateChecks
{
    ...
    private static async Task CallbackAbuseAsync(string root, string python, string operation)
    {
        ...
        await using var shared = await host.ShareAsync(Context(), profile, new() { Degree = 2, MaximumRestarts = operation == "duplicate" ? 0 : 1 }, callbacks, ["echo"]);
        await using var client = shared.For("abuse");
        string status;
        try { await client.CallAsync(operation, JsonSerializer.SerializeToElement(new { })); status = "ok"; }
        ...
        else
        {
            ...
            Check((await client.CallAsync("echo", JsonSerializer.SerializeToElement(new { }))).GetString() == "abuse", "Callback flood corrupted the channel.");
    ...
    private static async Task TenantChurnAsync(string root, string python)
    {
        ...
        if (sdk is not null) launch.AddRange(["--sdk-path", sdk]);
        var profile = new ProcessProfile(python, launch, trustedCode: true, reservedMemoryMiB: 128) { ReusePolicy = WorkerReusePolicy.Shared };
        await using var shared = await host.ShareAsync(Context(), profile, new() { Degree = 16 }, new CountingCallbacks(), []);

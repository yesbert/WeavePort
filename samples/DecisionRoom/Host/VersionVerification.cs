...
internal static class VersionVerification
{
    internal static async Task RunAsync(RuntimePaths runtime, RunConfiguration original, string directory, CancellationToken token)
    {
        ...
        async Task<RoomSnapshot> Run(string name, RunConfiguration c, bool resume = false) => await runner.RunAsync(c, new RunOptions(PathFor(name), Resume: resume), token);
        var v1 = await Run("one", config);
        var v2 = await Run("two", config with { PluginVersion = "2" });
        ...
        {
            var result = await Run("two-" + language, config with { PluginVersion = "2", Participants = config.Participants.Select(p => p with { Language = language }).ToArray() });
            Check(Wire.Serialize(result) == Wire.Serialize(v2), "version 2 " + language + " parity and artifact identity");
        }
        ...
        await runner.RunAsync(config with { PluginVersion = "1" }, new RunOptions(PathFor("paused"), PauseAfterFirst: true), token);

...
internal static class HttpChecks
{
    internal static async Task RunAsync(string config, string output)
    {
        ...
        {
            if (context.Request.Headers["X-Test-Key"] != key)
            {
                context.Response.StatusCode = 401;
                return;
            }
            if (!int.TryParse(context.Request.Query["kib"], out int kib) || kib is < 256 or > 131072 || kib % 64 != 0)
            {
                context.Response.StatusCode = 400;
                return;
            }
            if (!await admission.WaitAsync(0, context.RequestAborted))
            {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("Fixture endpoint must reject absent key.");
            }

            client.DefaultRequestHeaders.Add("X-Test-Key", key);
            await VerifyTransfersAsync(client, output);
        }
        finally
        {
            await app.StopAsync();
    ...
    private static async Task VerifyTransfersAsync(HttpClient client, string output)
    {
        var rows = new List<object>();
        var cases = from kib in new[] { 256, 8192, 32768, 131072 }
                    from fan in new[] { false, true }
                    select (kib, fan);
        foreach (var (kib, fan) in cases)
        {
            ...
            if (caller.Count != expected)
            {
                throw new InvalidDataException("HTTP caller length mismatch.");
            }
            ...
            rows.Add(new
            {
                inputKiB = kib,
                fanOut = fan,
                deliveredBytes = caller.Count,
                headersMs,
                completeMs = Stopwatch.GetElapsedTime(start).TotalMilliseconds,
                correctness = "all HTTP response bytes verified"
            });
            Console.WriteLine($"HTTP {kib} KiB fan={fan}: {Stopwatch.GetElapsedTime(start).TotalSeconds:F2}s");
        }

        await File.WriteAllTextAsync(Path.Combine(output, "http.json"), JsonSerializer.Serialize(new
        {
            transport = "loopback HTTP/1.1",
            tenant = "fixed test identity, not a production authentication scheme",

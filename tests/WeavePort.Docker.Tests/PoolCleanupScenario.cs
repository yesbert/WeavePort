...
{
    internal static Task RunAsync(Evidence evidence) => evidence.CheckAsync("pool: uncertain removal quarantines capacity until confirmed cleanup", async () =>
    {
        ...
        await CreateRemovalFailureWrapperAsync(directory, flag);
        ...
        try
        {
            ...
            IPluginSession a = await BindAsync("A");
            ...
            try
            {
                ...
                throw new Exception("Removal failure was hidden");
            }
            catch (IOException) { }
            ...
            VerifyEndpoint(endpoint, expected: true, "Unconfirmed removal deleted the endpoint");
            ...
            {
                throw new Exception("Quarantine admitted extra work");
            }

            File.Delete(flag);
            await host.MaintainAsync();
            VerifyEndpoint(endpoint, expected: false, "Confirmed removal retained the endpoint");

            if (host.Snapshot.Workers != 0)
            {
                throw new Exception("Confirmed cleanup did not release capacity");
            }
        ...
        Task<IPluginSession> BindAsync(string tenant) => host.BindAsync(new PluginContext(tenant, "demo", "1", "default", JsonSerializer.SerializeToElement(new
        {
        })),
            TestProfiles.Create("weaveport-poc-python:1"), new NoCallbacks(), []);
    });
    private static void VerifyEndpoint(string? endpoint, bool expected, string message)
    {
        if (endpoint is null)
        {
            return;
        }
        if (Directory.Exists(endpoint) != expected)
        {
            throw new InvalidDataException(message);
        }
    }

    private static async Task CreateRemovalFailureWrapperAsync(string directory, string flag)
    {
        using Process which = Process.Start(new ProcessStartInfo("which") { ArgumentList = { "docker" }, RedirectStandardOutput = true })!;
        string docker = (await which.StandardOutput.ReadToEndAsync()).Trim();
        await which.WaitForExitAsync();
        if (which.ExitCode != 0)
        {
        ...
        using Process chmod = Process.Start(new ProcessStartInfo("chmod") { ArgumentList = { "+x", wrapper } })!;

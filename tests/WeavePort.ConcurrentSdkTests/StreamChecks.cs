...
internal static class StreamChecks
{
    ...
    internal static ProcessProfile Profile(string root, string language)
    {
        ...
        if (sdk is not null) arguments = [.. arguments, "--sdk-path", sdk];
    ...
    private static async Task DeadlinesAsync(ProcessProfile profile, string language)
    {
        ...
        await using (var iterator = client.StreamAsync("slow", Json(new { firstDelay = 12000, nextDelay = 0 })).GetAsyncEnumerator())
        {
        ...
        try
        {
            await client.CallAsync("echo", Json(new { delay = 12000 }));
        ...
        await client.CallAsync("echo", Json(new { }));
        started = Stopwatch.GetTimestamp();
        await using var live = client.StreamAsync("slow", Json(new { nextDelay = 500 })).GetAsyncEnumerator();
    ...
    private static async Task PressureAsync(ProcessProfile profile, string language)
    {
        ...
        await using var iterator = stream.StreamAsync("slow", Json(new { nextDelay = 500 })).GetAsyncEnumerator();
    ...
    private static async Task CancelPausedAsync(ProcessProfile profile, string language)
    {
        ...
        var iterator = client.StreamAsync("slow", Json(new { nextDelay = 10000 }), stop.Token).GetAsyncEnumerator();
        ...
        int pid = (await client.CallAsync("echo", Json(new { }))).GetProperty("pid").GetInt32();
        await iterator.DisposeAsync();
        int after = (await client.CallAsync("echo", Json(new { }))).GetProperty("pid").GetInt32();
    ...
    private static async Task QueueCancellationAsync(ProcessProfile profile, string language)
    {
        ...
        await using var iterator = first.StreamAsync("slow", Json(new { nextDelay = 300 })).GetAsyncEnumerator();
        Check(await iterator.MoveNextAsync(), "First stream reserves sole worker");
        using var stop = new CancellationTokenSource(50);
        try
        {
            await using var waiting = second.StreamAsync("slow", Json(new { }), stop.Token).GetAsyncEnumerator();
    ...
    private static async Task WaitAsync(Func<bool> ready, string message)
    {
        ...
        while (!ready()) await Task.Delay(10, stop.Token);
        Check(ready(), message);
    }

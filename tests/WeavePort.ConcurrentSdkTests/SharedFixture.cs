...
internal static class SharedFixture
{
    internal static Task RunAsync(bool concurrent)
    {
        ...
        app.Function<JsonElement, object>("callbacks", CallbacksAsync);
        app.Function<JsonElement, string>("delay", DelayAsync);
        ...
        app.Function<JsonElement, object>("cpu", ComputeCpu);
        app.Stream<JsonElement, int>("items", (_, _, _) => Items());
        return app.RunAsync();
    }

    private static async ValueTask<object> CallbacksAsync(JsonElement input, PluginCallContext context, CancellationToken token)
    {
        ...
        {
            result = await context.CallHostAsync(operation, input, token);
        }
    ...
    private static async ValueTask<string> DelayAsync(JsonElement input, PluginCallContext context, CancellationToken _)
    {
    ...
    private static ValueTask<object> ComputeCpu(JsonElement input, PluginCallContext context, CancellationToken _)
    {

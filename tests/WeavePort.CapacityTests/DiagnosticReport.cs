...
internal static class DiagnosticReport
{
    internal static async Task WriteAsync(string directory)
    {
        ...
        foreach (string path in files)
        {
            ...
            if (!File.Exists(calibration) && File.Exists(calibration + ".gz")) calibration += ".gz";
            if (!File.Exists(calibration)) continue;
            ...
            {
                double offset = estimates[tenant.GetProperty("Tenant").GetInt32()].Offset;
                foreach (JsonElement observation in tenant.GetProperty("Observations").EnumerateArray())
                {
                    if (observation.GetProperty("Status").GetString() != "ok" || observation.GetProperty("WorkerTiming").ValueKind != JsonValueKind.Object) continue;

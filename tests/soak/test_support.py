...
class SupervisorTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (path / "result.json").write_text(
                json.dumps(
                    dict(
                        ...
                        sourceCommit="test",
                        scope="fixture",
                        settings=dict(clients=1),
                    )
                )
            )
            metric = dict(values=dict(rate=0.0000005, passes=1, fails=1999999))
            (path / "k6-summary.json").write_text(
                json.dumps(
                    dict(
                        metrics={
                            "unexpected_errors": metric,
                            "unexpected_errors{tenant:tenant-0}": metric,
                        }
                    )
                )
            )
            subprocess.run(
                [
                    sys.executable,
                    "-B",
                    str(Path(__file__).resolve().parents[2] / "tools/soak/analyze.py"),
                    directory,
                ],
                check=True,
                capture_output=True,
        ...
        for _ in range(100):
            self.assertFalse(
                cleanup_uncertain(
                    dict(
                        Quarantined=3,
                        OldestQuarantineSeconds=1,
                        MaintenanceFailure=None,
                    )
                )
            )
    ...
    def test_stuck_cleanup_and_maintenance_failure_abort(self):
        ...
        self.assertTrue(
            cleanup_uncertain(
                dict(
                    Quarantined=0,
                    OldestQuarantineSeconds=0,
                    MaintenanceFailure="IOException",
                )
            )
        )
        self.assertFalse(
            cleanup_uncertain(
                dict(Quarantined=0, OldestQuarantineSeconds=0, MaintenanceFailure=None)
            )
        )
    ...
    def test_invalid_cleanup_diagnostics_fail_closed(self):
        ...
        self.assertTrue(
            cleanup_uncertain(
                dict(Quarantined=0, OldestQuarantineSeconds=1, MaintenanceFailure=None)
            )
        )
        self.assertTrue(
            cleanup_uncertain(
                dict(Quarantined=-1, OldestQuarantineSeconds=0, MaintenanceFailure=None)
            )
        )
    ...
    def test_package_identity_rejects_changed_loaded_bytes(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            feed = Path(directory)
            version = ET.parse(
                Path(__file__).resolve().parents[2] / "Directory.Build.props"
            ).findtext(".//Version")
            with zipfile.ZipFile(
                feed / f"WeavePort.Sdk.{version}.nupkg", "w"
            ) as archive:
                archive.writestr("lib/net10.0/WeavePort.Sdk.dll", b"expected")
            verify_package(
                feed, "WeavePort.Sdk", hashlib.sha256(b"expected").hexdigest()
            )
            with self.assertRaises(RuntimeError):
                verify_package(
                    feed, "WeavePort.Sdk", hashlib.sha256(b"changed").hexdigest()
                )
    ...
    def test_resource_accounting_retains_first_last_and_peak(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                "\n".join(
                    json.dumps({"processes": [{"rssKiB": value}]})
                    for value in (1024, 4096, 2048)
                )
            )

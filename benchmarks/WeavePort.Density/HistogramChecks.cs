...
internal static class HistogramChecks
{
    internal static void Run()
    {
        ...
        foreach (double value in values.Skip(500))
        {
            second.Add(value);
        }
        ...
        foreach (double q in new[] { .5, .95, .99 })
        {
            ...
            if (actual < expected || actual > expected * 1.010001)
            {
                throw new InvalidOperationException("Incorrect merged percentile");
            }
        }
        ...
        if (new Histogram().Quantile(.99) != 0)
        {
            throw new InvalidOperationException("Incorrect empty percentile");
        }

        long began = GC.GetAllocatedBytesForCurrentThread();
        var sparse = Enumerable.Range(0, 1000).Select(_ => new DensityStats()).ToArray();
        foreach (var client in sparse)
        {
            client.Latency.Add(1);
            client.Execution.Add(1);
        }
        long allocated = GC.GetAllocatedBytesForCurrentThread() - began;
        GC.KeepAlive(sparse);
        if (allocated > 4 * 1048576)
        {
            throw new InvalidOperationException("Sparse customer recorder allocates excessive memory");
        }

        Console.WriteLine(JsonSerializer.Serialize(new
        {
            passed = true,
            allocatedBytesFor1000TwoSampleCustomers = allocated
        }));
    }
}

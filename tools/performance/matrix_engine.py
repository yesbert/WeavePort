...
def request(path, method, route, body=None):
    ...
    try:
        ...
        connection.request(method, route, body=encoded, headers={'Content-Type': 'application/json'})
        ...
        if not 200 <= response.status < 300:
            raise RuntimeError('Engine HTTP ' + str(response.status) + ': ' + data.decode(errors='replace')[:200])
...
class EngineLane(MatrixLane):
    ...
    async def start(self):
        ...
        body = {'Image': self.image, 'Cmd': ['trusted' if self.mode == 'fresh' else self.mode],
                'User': '0:0' if fork else '65532:65532', 'OpenStdin': True, 'StdinOnce': False,
                'AttachStdin': True, 'AttachStdout': True, 'AttachStderr': True, 'Tty': False,
                'Labels': {'weaveport.matrix-experiment': 'true'},
                'HostConfig': {'Init': True, 'NetworkMode': 'none', 'IpcMode': 'private', 'ReadonlyRootfs': True,
                    'CapDrop': ['ALL'], 'CapAdd': ['SETUID', 'SETGID', 'KILL'] if fork else [],
                    'SecurityOpt': ['no-new-privileges'], 'Memory': self.memory * 1048576,
                    'MemorySwap': self.memory * 1048576, 'NanoCpus': int(float(self.cpus) * 1000000000),
                    'PidsLimit': 64, 'Tmpfs': tmpfs, 'ShmSize': 8 * 1048576, 'LogConfig': {'Type': 'none'}}}
        await asyncio.to_thread(request, self.engine.path, 'POST', '/containers/create?name=' + self.name, body)
        reader, writer = await asyncio.open_unix_connection(self.engine.path, limit=1048576)
        ...
        route = '/containers/' + self.name + '/attach?stream=1&stdin=1&stdout=1&stderr=1'
        writer.write(('POST ' + route + ' HTTP/1.1\r\nHost: localhost\r\nConnection: Upgrade\r\nUpgrade: tcp\r\nContent-Length: 0\r\n\r\n').encode())
        await writer.drain()
        ...
        await asyncio.to_thread(request, self.engine.path, 'POST', '/containers/' + self.name + '/start')
    ...
    async def close(self):
        if self.name is None:
            return
        await asyncio.to_thread(request, self.engine.path, 'DELETE', '/containers/' + self.name + '?force=true')

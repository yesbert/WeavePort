...
- One client binding is single-flight for the entire enumeration. A second concurrent call fails busy. Multiple independent bindings provide parallelism; products orchestrate serial or fan-out/fan-in work. Customer-bound workers never move between tenants. Explicitly approved native SDK deployments can reuse a worker after registered session cleanup; see [resource ownership and best practices](reusable-plugins.md).

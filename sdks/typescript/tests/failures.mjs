...
test('all cleanup causes and primary failure remain observable', async () => {
    ...
    await assert.rejects(
        executeWithCleanup(action, async () => {}),
        (error) => error === primary,
    );
});
test('decode rejects malformed protocol boundaries and preserves valid frames', () => {
    for (const frame of [
        ...
        {},
        { type: 'invoke' },
        { type: 'configure', degree: 1025 },
        { type: 'callback-result', id: 'a', callbackId: 1 },
    ]) {
        assert.throws(() => decodeFrame(frame));
    }
    assert.deepEqual(decodeFrame({ type: 'configure', degree: 16 }), {
        type: 'configure',
        degree: 16,
    });
    assert.equal(
        decodeFrame({
            ...
            id: 'x',
            operation: '$sdk.call',
            payload: { operation: 'echo', input: 1 },
            context: { tenant: 'a', configuration: {} },
        }).payload.input,
        1,
    );
});

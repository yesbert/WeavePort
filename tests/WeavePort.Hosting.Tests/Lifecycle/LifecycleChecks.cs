...
internal static class LifecycleChecks
{
    ...
    internal static async Task WorkerAsync()
    {
        ...
        {
            await ReplyToWorkerRequestAsync(line);
        }
    }

    private static async Task ReplyToWorkerRequestAsync(string line)
    {
    ...
    private static async Task<int> CancellationAsync(string root)
    {
        ...
        for (int i = 0; i < 100 && host.Snapshot.Tenants != 0; i++)
        {
            await Task.Delay(10);
        }
    ...
    private static async Task<int> StartupAndCleanupAsync()
    {
        ...
        for (int i = 0; i < 100 && !log.Events.Any(item => item.Id == 1005); i++)
        {
            await Task.Delay(20);
        }

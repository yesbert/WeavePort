...
internal static class LinuxReport
{
    internal static async Task WriteAsync(string directory)
    {
        ...
        foreach (string name in new[] { "stop-reason.txt", "exit-code.txt", "outer-exit-code.txt" })
        {
            ...
            text.AppendLine($"- {name}: {File.ReadAllText(Path.Combine(directory, name)).Trim()}");
        }
        ...
        AppendResources(text, resourceFile);
    ...
    private static void AppendStage(StringBuilder text, string file)
    {
        ...
        if (requests != row.GetProperty("Requests").GetInt64() || errors != row.GetProperty("Errors").GetInt64())
        {
            throw new InvalidDataException("Raw outcomes disagree with aggregate counts");
        }
    ...
    private static void AppendResources(StringBuilder text, string resourceFile)
    {
        if (!File.Exists(resourceFile))
        {
            return;

...
internal static class CompositionChecks
{
    internal static async Task RunAsync(string root)
    {
        await using (var scope = new ResultScope(root, "A", new(4096, 8192, 3)))
        {
            ...
            var fake = new BoundClient("B", Check.Json(new
            {
                data = ""
            }));
            ...
            var mapped = await Composition.MapAsync(scope, input, new BoundClient("A", Check.Json(new
            {
                data = new byte[4096]
            })), 4096);
    ...
    private sealed class BoundClient(string tenant, JsonElement reply) : IBoundPluginClient
    {
        ...
        public Task<JsonElement> CallAsync(string operation, JsonElement input, CancellationToken cancellationToken = default)
        {
            Calls++;

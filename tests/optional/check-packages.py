...
expected = json.loads((root / 'compatibility/optional-dependencies.json').read_text())
for name, dependencies in expected['packages'].items():
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        assert value('readme') in archive.namelist() and value('icon') in archive.namelist()
        actual = {d.attrib['id']: d.attrib['version'] for d in spec.findall('.//{*}dependency')}
        assert actual == {k: v.replace('$version', version) for k, v in dependencies.items()}, (name, actual)
        assert not any('Grpc.Tools' in n or n.endswith('/protoc') for n in archive.namelist())
        if name.endswith('.Client'):
            assert not spec.findall('.//{*}frameworkReference'), 'Client requires shared hosting framework'
...
for project, dependencies in expected['sourceClosures'].items():
    ...
    actual = {key: value for key, value in assets['libraries'].items() if value['type'] == 'package'}
    ...
    for key, license_name in dependencies.items():
        ...
        package = next(Path(folder) / relative for folder in assets['packageFolders'] if (Path(folder) / relative).exists())

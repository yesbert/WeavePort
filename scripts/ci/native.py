...
for transport in (("stdio",) if os.name == "nt" else ("stdio", "socket")):
    ...
    config = {
        **configuration,
        "WorkspaceRoot": str(evidence / "workspaces"),
        "UseUnixSocket": transport == "socket",
    }
    config_path = evidence / "config.json"
    config_path.write_text(json.dumps(config, indent=2))
    run(
        "dotnet",
        "tests/WeavePort.Local.Tests/bin/Release/net10.0/WeavePort.LocalDemo.dll",
        "verify",
        config_path,
        evidence,
    )

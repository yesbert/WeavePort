...
internal static class McpMeasurements
{
    ...
    internal static async Task RunAsync(string executable, string fixtureRoot, string output)
    {
        ...
        try
        {
            for (int repeat = 0; repeat < 5; repeat++)
            {
                ...
                if (repeat % 2 != 0) Array.Reverse(protocols);
                foreach (var protocol in protocols)
                {
                    ...
                    await using var session = await host.BindAsync(new PluginContext("measure", "echo", "1", "measure", JsonSerializer.SerializeToElement(new { })), profile, new Callbacks(), []);
                    long start = Stopwatch.GetTimestamp();
                    await Echo(session, protocol, "start");
                    ...
                    foreach (int width in new[] { 16, 65536 })
                    {
                        ...
                        for (int i = 0; i < 30; i++) await Echo(session, protocol, text);
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            await Echo(session, protocol, text);
                            samples[i] = Stopwatch.GetElapsedTime(before).TotalMilliseconds;
                        }
                        allocated = GC.GetTotalAllocatedBytes(true) - allocated;
                        Array.Sort(samples);
                        runs.Add(new { protocol = protocol.ToString(), repeat, width, count, firstCallMs, meanMs = samples.Average(), p50Ms = samples[count / 2], p99Ms = samples[(int)(count * .99)], callerAllocatedBytesPerCall = (double)allocated / count, samples });
                    }
                }
            }
    ...
    private static async Task Echo(IPluginSession session, ProcessProtocol protocol, string text)
    {
        var arguments = new { text };
        var payload = protocol == ProcessProtocol.Native
            ? JsonSerializer.SerializeToElement(new { operation = "echo", input = arguments })
            : JsonSerializer.SerializeToElement(new { name = "echo", arguments });
        var result = await session.InvokeAsync(protocol == ProcessProtocol.Native ? "$sdk.call" : "tools/call", payload);
        if (result.Status != "ok") throw new Exception("Measurement call failed: " + result.Status);
        var value = protocol == ProcessProtocol.Native ? result.Value : result.Value.GetProperty("structuredContent");
        if (value.GetProperty("text").GetString() != text) throw new Exception("Incorrect complete result");
    }

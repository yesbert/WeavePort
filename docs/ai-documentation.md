...
version (for 0.3.0: v0.3.0), and keep all source reads on that revision.
...
Verify the connection by requesting `get_file_contents` with `owner: yesbert`, `repo: WeavePort`, `path: docs/status.md` and `ref: refs/tags/v0.3.0`. The result should contain the file from that revision. Tool prefixes vary by client. A missing tool usually means the server is not started or its repository tools are disabled; authentication failures need the client's sign-in or token setup. These setup instructions are based on GitHub's documentation; they are not a recorded authenticated client test.
...
The separate [local MCP plugin integration](mcp-plugins.md) lets current source consume MCP tools inside an application. It is not a documentation retrieval service or an exported MCP gateway.

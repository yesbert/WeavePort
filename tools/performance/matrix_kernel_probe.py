...
from reuse_support import request, command, write
...
async def main():
    ...
    for mode in ["fresh", "process", "trusted", "forkserver"]:
        ...
        try:
            ...
            probe = await lane.call(
                request(2, op="kernel_probe", shmid=value.get("shmid", -1))
            )
    ...
    write(
        ...
        {"known": sorted(known), "remaining": sorted(known & names)},
    )
    if known & names:

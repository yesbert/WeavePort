...
def extract(archive, destination):
    with tarfile.open(archive, "r:gz") as tar:
        ...
        if (
            len(members) > MAX_FILE_COUNT
            or sum(m.size for m in members) > MAX_CONTENT_BYTES
        ):
        ...
        for member in members:
            ...
            if any(part.startswith(".") for part in path.parts) or (
                member.isfile() and path.suffix not in ALLOWED
            ):
    ...
    if (
        not re.fullmatch("[0-9a-f]{40}", metadata.get("revision", ""))
        or type(metadata.get("run")) is not int
        or metadata["run"] < 1
    ):
...
def extract_member(tar, member, destination):
...
def verify_run_order(current, metadata):
    receipt = current / "deployment.json"
...
def retain_release(candidate, release):
    if not release.exists():
        candidate.rename(release)
        return
    if content_identity(release) != content_identity(candidate):
...
def receive(root, stream):
    ...
    with (root / "deploy.lock").open("a") as lock:
        ...
        with tempfile.TemporaryDirectory(dir=root) as temporary:
            ...
            verify_run_order(current, metadata)
            # The server owns cache policy; archives cannot upload .htaccess.
            (candidate / ".htaccess").write_text(
                '<IfModule mod_headers.c>\nHeader set Cache-Control "no-cache"\n</IfModule>\nAddType text/markdown .md\n'
            )
            release = (
                root / "releases" / (str(metadata["run"]) + "-" + metadata["revision"])
            )
            release.parent.mkdir(exist_ok=True)
            retain_release(candidate, release)
            next_link = root / "next"

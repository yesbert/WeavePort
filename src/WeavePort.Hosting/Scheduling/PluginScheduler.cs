...
internal sealed partial class PluginScheduler : IAsyncDisposable
{
    ...
    internal Task<ScheduledInvocationResult> Enqueue(ScheduledPlugin plugin, string operation, JsonElement payload, CancellationToken token)
    {
        ...
        lock (_sync)
        {
            ...
            {
                return AdmitImmediately(call, plugin);
            }
    ...
    private string? AdmissionFailure(ScheduledPlugin plugin, CancellationToken token)
    {
        ...
        {
            return FailureCodes.Denied;
        }

        return _failure is not null || _queue.Count >= _options.MaximumQueuedCalls || _queue.Count(c => c.Plugin.Tenant == plugin.Tenant) >= _options.MaximumQueuedCallsPerTenant ? FailureCodes.Busy : null;
    }
    ...
    private Task<ScheduledInvocationResult> AdmitImmediately(ScheduledCall call, ScheduledPlugin plugin)
    {
        if (!Eligible(plugin, _active.ToArray()) || !Fits(plugin) || !Admit(call))
        {
            _rejected++;
            return Task.FromResult(ScheduledCall.Rejected(FailureCodes.Busy));
        }

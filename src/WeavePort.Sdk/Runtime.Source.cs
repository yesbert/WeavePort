...
internal sealed partial class Runtime
{
    ...
    private async Task<object> DispatchSourceAsync(string operation, JsonElement payload, CancellationToken token)
    {
        if (operation == SdkOperations.SourceOpen)
        {
            ...
            _source = await application.Sources[payload.GetProperty("operation").GetString()!](payload.GetProperty("input"), context, token);
            if (!_source.CanRead)
            {
        ...
        if (_source is null || payload.GetProperty("source").GetString() != _sourceId)
        {
        ...
        int chunkBytes = payload.GetProperty("chunkBytes").GetInt32();

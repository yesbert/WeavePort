...
internal static class CallMetadataChecks
{
        ...
        try
        {
            ...
            foreach (IPluginClient client in new IPluginClient[] { local, remote })
            {
                ...
                for (int index = 0; index < results.Length; index++)
                    if (results[index].Value != index + 1 || results[index].ElapsedMs != (index + 1) * 10.5) throw new Exception("Per-call metadata changed.");
                try { await client.CallWithMetadataAsync<int, int>("failure", 0); throw new Exception("Failure accepted"); }
                ...
                try { await client.CallWithMetadataAsync<int, int>("mismatch", 0); throw new Exception("Mismatch accepted"); }
            ...
            foreach (IPluginClient client in new IPluginClient[] { new LegacyClient(), legacyRemote })
            {
                ...
                if (result.Value != 42 || result.ElapsedMs is not null) throw new Exception("Legacy timing fabricated.");
            }
        }
    ...
    private sealed class TimingSession : IPluginOperationSession
    {
        ...
        {
            if (payload.GetProperty("operation").GetString() == "mismatch") return new("version-mismatch", JsonSerializer.SerializeToElement(new { }), Instance, 1) { VersionMismatch = new("1.0.0", "1") };
            if (payload.GetProperty("operation").GetString() == "failure") return new("failed", JsonSerializer.SerializeToElement(new { }), Instance, 1, true) { Failure = new("sdk-error", "exchange", "correlation-fixture", true) };
